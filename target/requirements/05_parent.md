...
> `Edit`, and `Delete` — the `Document | Text` toggle sits **inline in the card header row** (BR-EXT-046). Video and text rows render as single rows with a Draft/Published pill and, when draft, an inline **Publish** button (unpublish deferred).

### Original P-topic specification (unchanged, now rendered inline)

- Topic title (editable).
- Content actions: same Add Content modal as Platform Admin (PDF / Image / Video / Text). Text uses the shared editor with **Code | Split | Preview** modes and fullscreen, LaTeX in the preview, a `n / 20,000` counter, **`.md` import**, and a discard-changes guard (BR-EXT-048/049/050/051/052); Edit sends only changed fields (BR-EXT-047). PDF and Image save the uploaded document **instantly** as a permanent raw row and *then* trigger the extraction pipeline for the editable extracted-text rows; Video and Text save instantly.
- **Extraction is optional, not required (BR-EXT-038).** The uploaded PDF/image is viewable and publishable the moment upload returns — before the worker picks it up, and permanently even if extraction terminally fails (vision-LLM / OCR backend down). A parent who only wants their child to read the document, with no AI tutor grounding over it, never depends on the extraction pipeline: they simply publish the Document side. A failed job shows as an **advisory** in the status strip with a Retry action, never as a blocker, and the content row stays usable throughout. There is no separate "no AI tutor" opt-in — publishing the Document side *is* that choice, and raw rows are never enqueued for RAG indexing by construction (BR-DATA-025).
...
- **Content viewers:** parent can view every upload in both its raw form (PDF viewer / image viewer) and its extracted, editable text form before deciding what students see — same shared `ContentViewer`, and the same view dialog, that students use (`target/requirements/12_content_extraction.md` § Content list, then viewer). The PDF viewer carries fit-width, zoom, page-navigation and fullscreen controls; the image viewer carries fit, zoom and fullscreen; the text viewer carries text size and fullscreen and renders LaTeX/chemistry math — one shared toolbar (BR-EXT-044, BR-EXT-050); the PDF page number is typeable to jump directly (BR-EXT-053).
- **Publish control:** each upload group gets a Publish action — for PDF/Image, a mutually-exclusive "Publish as Document" (raw) vs. "Publish as Text" (extracted) toggle; for Video/Text, an inline **Publish** button in the row (unpublish is deferred — a published row shows only its pill). Controls are inline in their row, never a separate strip; the topic row's Draft/Live toggle is the only topic-level publish, and the content header warns when the topic is Live but items are still drafts (BR-EXT-046). While extraction is still running, and permanently after a terminal failure, the group has no text side: "Publish as Text" renders **disabled** and "Publish as Document" stays available (BR-DATA-024 degenerate group). Nothing is visible to the linked child until published, independent of the topic's own Draft/Live status (BR-PAR-021).
...
- BR-PAR-008a: Parent extraction quota — max 5 concurrent jobs (`status IN ('pending','extracting')`) and max 100 jobs/day. Enforced application-layer via `parent_quota_counters` row lock inside the POST handler TX. APISIX rate limit (50/day per parent token) is a coarse second-line defence. **Status note (2026-09-14): this quota has never actually been enforced.** `increment_quota` only flushes and was called after the committing call, so the counter was never persisted and both caps were inert — see BR-EXT-039 and `Implementation_planning/constraints.md`. It becomes live once the ordering fix lands, at which point **the concurrent counter must also decrement on every terminal transition and re-increment on retry (BR-EXT-040)**, or a run of failed extractions leaks all 5 slots and locks the parent out of uploading entirely. Treat the APISIX rate limit as the only limit that has been holding until then.
