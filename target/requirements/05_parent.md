...
- Content actions: same Add Content modal as Platform Admin (PDF / Image / Video / Text). Text uses the wide split Write | Preview editor with a `n / 20,000` counter and can **import a `.md` file** into the editor (BR-EXT-036/048/049); Edit sends only changed fields (BR-EXT-047). PDF and Image save the uploaded document **instantly** as a permanent raw row and *then* trigger the extraction pipeline for the editable extracted-text rows; Video and Text save instantly.
