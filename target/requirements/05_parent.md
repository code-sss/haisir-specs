...
- Content actions: same Add Content modal as Platform Admin (PDF / Image / Video / Text). PDF and Image save the uploaded document **instantly** as a permanent raw row and *then* trigger the extraction pipeline for the editable extracted-text rows; Video and Text save instantly.
...
- **Content viewers:** parent can view every upload in both its raw form (PDF viewer / image viewer) and its extracted, editable text form before deciding what students see — same shared `ContentViewer`, and the same view dialog, that students use (`target/requirements/12_content_extraction.md` § Content list, then viewer). The PDF viewer carries fit-width, zoom, page-navigation and fullscreen controls (BR-EXT-044).
- **Publish control:** each upload group gets a Publish action — for PDF/Image, a mutually-exclusive "Publish as Document" (raw) vs. "Publish as Text" (extracted) toggle; for Video/Text, a simple Draft/Published toggle. While extraction is still running, and permanently after a terminal failure, the group has no text side: "Publish as Text" renders **disabled** and "Publish as Document" stays available (BR-DATA-024 degenerate group). Nothing is visible to the linked child until published, independent of the topic's own Draft/Live status (BR-PAR-021).
