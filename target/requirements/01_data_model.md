...
**`text` length (2026-09-22):** the column is unbounded `VARCHAR` (no migration needed for any cap); the cap lives in the Pydantic schemas and is **20,000 characters** (raised from 4,000 — BR-EXT-048 in `12_content_extraction.md`). Markdown `.md` files imported through the Text editor land here as ordinary `text` rows — no new `content_type` (BR-EXT-049).
...
An upload materializes: (a) **one** raw content row (`content_type='pdf'` or `'image'`, matching `extraction_jobs.source_type`) pointing at a permanently-retained copy of the source file, and (b) the **N** extracted `content_type='text'` rows, one per page. Both sets share the same `source_extraction_job_id`. The raw row's file path is stored in the existing **`url`** column — the same column `video` and manually-uploaded `pdf` rows already use — so no new storage column is added. Both the raw row and the text rows default to `visibility_status='draft'`; see BR-DATA-024 for how one is chosen to publish.

**The raw row is created at upload time, not at finalize (revised — Extraction-Optional increment).** It is inserted by the POST handler in the same transaction as the `extraction_jobs` row, *before* the worker has touched the file. The **N** text rows are still materialized by the worker at finalize. This decouples "the parent can see and publish the document they uploaded" from "the extraction pipeline succeeded" — see BR-EXT-038. Superseded rule: the raw row used to be created inside the finalize TX, so a failed extraction produced **no** `topic_contents` row at all and the upload was unreachable by any endpoint until its source file was TTL-purged.

**Ordering:** the raw row is **appended after** the text rows (`order = N`, where text rows keep their existing `order = page_no`, 0-indexed). Because the raw row is now created before the page count is known, it is inserted at the stated constant **`order = 0`** and the finalize TX **UPDATEs it to `order = len(pages)`** (match key: `source_extraction_job_id` + `content_type IN ('pdf','image')`). A raw row whose job never reaches finalize therefore keeps `order = 0`. Parallel uploads to one topic may collide on `order`; there is no unique constraint and ties are broken arbitrarily — this is tolerated, not fixed here. Text-row ordering is deliberately left untouched because `provenance.page_no` is derived directly from `topic_contents.order` (`TopicContentRepository._set_provenance`) — shifting text rows to make room at the front would silently renumber every provenance badge. Uploader UIs that want the raw form listed first should sort client-side rather than encode it in `order`.

**BR-DATA-009 — The job's working copy is transient; the audit and the published raw file are permanent:**
`extraction_jobs.source_path` — the job's own working-directory copy of the upload — and the `extraction_jobs` row itself are still purged per status TTL, unchanged by BR-DATA-008. This purge is independent of the permanent raw `topic_contents` row (content_type `'pdf'`/`'image'`), whose file is a separate copy made **at upload time** (BR-DATA-008) and is never subject to this TTL — it persists until the content row itself is deleted. The accepted 2× disk cost (job working copy + permanent copy) therefore begins at upload rather than at extraction success. `extraction_job_audit` rows are **never purged** — they preserve provenance for materialized `topic_contents` regardless of which representation (raw or text) was ultimately published.

**Provenance gap for failure-surviving raw rows (accepted, stated deliberately):** `extraction_job_audit` is written **only** inside the finalize TX. A raw row that survives a terminal `extraction_failed` therefore has **no audit row**, so BR-EXT-021's filename badge is unresolvable for it and the row carries **no provenance badge**. This is intentional: "Extracted from `x.pdf` · page N" is meaningless when nothing was extracted, and the raw row's `title` is already the source filename, so upload identity is not lost. A successful retry of that job writes the audit row and the badge appears on the text rows as normal.
...
**Degenerate group — raw row with no text side:** because the raw row is created at upload (BR-DATA-008), an upload group legitimately exists with the raw row alone — while extraction is still running, and permanently if extraction terminally failed. The group keeps `kind='extraction'` (it has a non-null `source_extraction_job_id`); the toggle renders **"Publish as Document" enabled and "Publish as Text" disabled**, because there is no text row to address. Mutual exclusivity is trivially satisfied. The same shape applies to a 0-page successful extraction, which previously produced nothing at all.
