...
- **Content viewers:** admin can view every upload in both its raw form (PDF viewer / image viewer) and its extracted, editable text form — same shared `ContentViewer`, and the same view dialog, that students use (`target/requirements/12_content_extraction.md` § Content list, then viewer). The PDF viewer carries fit-width, zoom, page-navigation and fullscreen controls; the image viewer carries fit, zoom and fullscreen; the text viewer carries text size and fullscreen and renders LaTeX/chemistry math — one shared toolbar (BR-EXT-044, BR-EXT-050); the PDF page number is typeable to jump directly (BR-EXT-053).
- **Content publish control:** each upload group gets its own Publish action, independent of the topic's Draft/Live status — for PDF/Image, a mutually-exclusive "Publish as Document" (raw) vs. "Publish as Text" (extracted) toggle; for Video/Text, an inline **Publish** button in the row (unpublish deferred). Controls sit inline in their row, never as a separate strip; the content header warns when the topic is Live but items are still drafts ("students can't see them until you publish them") (BR-EXT-046). While extraction is running, and permanently after a terminal failure, the group has no text side: "Publish as Text" renders **disabled** and "Publish as Document" stays available (BR-DATA-024 degenerate group). A topic can be `live` while individual content items are still in draft, pending admin review (BR-ADM-008).

### Add Content modal (Phase 1d-real)

Replaces the URL-only stub shipped in Phase 1d. Native `<dialog>`, 560 px wide.

- Type chip selector: **PDF** (retained raw + extracted to text) · **Image(s)** (retained raw + OCR via vision LLM) · **Video URL** (YouTube/Vimeo) · **Text** (paste/write markdown, or import a `.md` file — BR-EXT-049; shared editor with Code | Split | Preview modes + fullscreen and LaTeX preview, 20,000-char cap with live counter, discard-changes guard — BR-EXT-048/050/051/052).
