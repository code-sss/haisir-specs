...
| Content list | **One row per published item** — type icon (📄 pdf / 🖼️ image / 🎬 video / 📝 text), title, **View** button. Rows only; no item body renders in the list. Revised (Upload & Viewer Feedback increment) — this panel used to render every item inline and stacked, so a topic with two PDFs rendered two whole documents on top of each other. See BR-EXT-045. |
| Content viewer | Opened by View, one item at a time, in a near-fullscreen `<dialog>` (≈90vw × 90vh, max 1200px) with the item title + close in its header; Esc and backdrop close, focus trapped. Dispatches per `content_type`: PDF viewer **with the BR-EXT-044 toolbar** (fit-width default, zoom ±, page prev/next + `n of N`, fullscreen), image viewer **with fit / zoom / fullscreen**, rendered-markdown viewer **with text size / fullscreen and LaTeX + chemistry math** (text, BR-EXT-050); the PDF page number is typeable to jump (BR-EXT-053) — one shared toolbar (BR-EXT-044 revised), YouTube/Vimeo SDK player with external-link fallback (video). Same dialog the parent/admin View button opens. |
| Doubt panel | Stays below the content list, outside the dialog — it is topic-scoped, not item-scoped |
