...
| Content viewer | Opened by View, one item at a time, in a near-fullscreen `<dialog>` (≈90vw × 90vh, max 1200px) with the item title + close in its header; Esc and backdrop close, focus trapped. Dispatches per `content_type`: PDF viewer **with the BR-EXT-044 toolbar** (fit-width default, zoom ±, page prev/next + `n of N`, fullscreen), image viewer **with fit / zoom / fullscreen**, rendered-markdown viewer **with text size / fullscreen** (text) — one shared toolbar (BR-EXT-044 revised), YouTube/Vimeo SDK player with external-link fallback (video). Same dialog the parent/admin View button opens. |
