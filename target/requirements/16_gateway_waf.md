...
- **BR-WAF-016 — Topic-content create (POST) matches the edit (PATCH) exclusion set, field-scoped.** The POST create chains (`id:199100` admin, `id:199121` parent) exclude six rules on `json.(title|description|text)`; the PATCH chain (`id:199120`) excludes eight — `942410`, `932200` and `942440` were added to PATCH only, after live dev failures on OCR'd LaTeX + lettered MCQ bodies (recorded in the chain's own justification comment); markdown `##` / `---` are the likely trigger for `942440` (SQL comment sequences) — the harness below confirms which. With LaTeX preview and `.md` import (BR-EXT-049/050), authored math and headings now arrive on **create** as often as on edit, so the same false positives would 403 a new text item. Add the three to both POST chains in the same `ctl:ruleRemoveTargetById=<id>;ARGS_POST:/^json\.(title|description|text)$/` form. **First tighten `id:199100`'s URI match** from `@beginsWith /api/topics-contents/` to `@rx ^/api/topics-contents/?$`, so no exclusion widens to POST sub-paths such as `/publish`. Harness corpus (BR-WAF-011), asserting on rule IDs: inline and display fractions, `\begin{aligned} … \\[4pt] … \end{aligned}`, `\ce{A <=> B}`, `$0<i<n$`, markdown headings, `---` rules, tables, lettered `(a) (b)` options — all 200 on POST and PATCH; the same strings in a query arg or header still 403. XSS `941160` and the libinjection detectors stay active on these fields and must not be excluded; `942430` (score 3) stays active and the corpus must include a dense `aligned` block to show it does not combine with another warning to reach 5. Correct the stale `03-secured-api.json` comment that says field-scoped `932200` does not work (BR-WAF-008 — true on Coraza < 3.5, not on 3.7).
