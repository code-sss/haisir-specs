...
**Also complete (2026-06-16 — G8 backend + G9 backend):** `GET /api/student/nodes` now returns a fully nested `PlatformNodeCard` tree: `get_all_platform_nodes_visible(viewer_sub)` fetches all `owner_type='platform'` nodes across categories; `_build_node_tree()` (extracted as a module-level utility from `CoursePathNodeService`) assembles them. `PlatformNodeCard` schema gains `children: list[PlatformNodeCard]` (recursive self-reference via `model_rebuild()`). `topic_count` is now real — `get_topic_counts_for_nodes(node_ids)` runs a single GROUP BY on `status='live'` topics and is wired into both `get_dashboard()` and `get_node_tree()`. Unit test (`TestGetNodeTreePlatform`, grade → subject → course seed) and integration test (depth ≥ 2, parent-owned excluded) added. SonarQube: LM Studio placeholder key hoisted (`8097f9d`), S5332 false positive suppressed (`04a96f1`). T8.1–T8.7 and T9.1–T9.3 complete.

**Also complete (2026-06-17 — G8 gate + G11 manual sign-off):** NodeTreeSidebar updated in frontend (`31062ab`) to consume recursive `children` from `GET /api/student/nodes` — expandable grade ▶ → subject ▶ → course hierarchy confirmed in browser. Manual walkthrough (G11): student logs in, expands tree to Maths → Arithmetic → selects "Ratio" topic, LaTeX-formatted exam content renders correctly in ContentViewer (T11.2 ✓). Home Study tab shows "No Home Study content yet — ask your parent to link their account" placeholder with no parent link (T11.3 ✓). **Known bug (G9 gate pending):** `topic_count` shows 0 on all home cards and sidebar nodes — root cause is `get_topic_counts_for_nodes` counting only direct topics; grade/subject nodes have 0 direct topics. Fix: switch to recursive CTE (subtree sum) — backend fix in progress, pending sign-off.

> Snapshot baseline: haisir-backend `04a96f1` (feature/rag, SonarQube fixes on rag_outbox_loop, 2026-06-16), haisir-frontend `31062ab` (feature/rag, G8 frontend + G11 sign-off, 2026-06-17), haisir-deploy `e57c56b` (feature/rag, all RAG env vars wired, 2026-06-15).
> Next session: `git diff 04a96f1..HEAD` in haisir-backend, `git diff 31062ab..HEAD` in haisir-frontend, `git diff e57c56b..HEAD` in haisir-deploy.

**Not yet built:** Parent curriculum builder (adopt board subtree, create own nodes/topics, upload notes), parent link-code generation and redemption. G9 `topic_count` display fix (recursive subtree count — backend fix in progress, pending sign-off). Playwright E2E tests deferred across all phases (Playwright not installed).
