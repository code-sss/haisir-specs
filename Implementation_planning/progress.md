...
> Snapshot baseline: haisir-backend `0e6c7e8` (chore(deps): upgrade all dependencies and pre-commit hooks, 2026-04-22), haisir-frontend `7633f19` (feat(admin): add extraction status panel, toast system, and upload fixes, 2026-05-05), haisir-deploy `baa20bf` (fix(deploy): auto-restart services whose image_tag is overridden in manifest, 2026-04-22).
> Next session: `git diff 0e6c7e8..HEAD` in haisir-backend, `git diff 7633f19..HEAD` in haisir-frontend, and `git diff baa20bf..HEAD` in haisir-deploy instead of re-reading the full codebases.
