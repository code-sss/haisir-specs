...
**Also complete (2026-06-15 — feature/rag backend: Phase 2 RAG drain loop + text restructuring + student dashboard):** Full RAG pipeline wired on `feature/rag`. V31 Alembic migration enables `vector` extension (T2.1); V32 adds `data_topic_content_chunks` table with `embedding vector(1024)` (T2.2). LlamaIndex deps + `OllamaEmbedding(bge-m3)` in `pyproject.toml` (T3.1). `EmbeddingSettings` with `embed_dim: int = Field(default=1024)` (T3.2). `rag_outbox_loop.py`: `PGVectorStore.from_params(hybrid_search=True, text_search_config="english", hnsw_kwargs={m:16, ef_construction:64, ef_search:40})` + 3-level hierarchy JOIN (`topic_contents → topics → course_path_nodes`) storing `topic_title`, `node_name`, `parent_name`, `grandparent_name`, `page_order` in chunk metadata; `VectorStoreIndex.from_vector_store()` + `index.insert_nodes(nodes)` pattern (not `vector_store.add`) (T3.3); registered in `worker/__main__.py` (T3.4); unit tests assert `hybrid_search=True`, HNSW params, hierarchy metadata (T3.6); integration test seeds full hierarchy and asserts `text_search_tsv` column exists (T3.7). `HaituSettings` with all 8 fields: `model_spec`, `ollama_base_url`, `max_tokens`, `top_k`, `rerank_model`, `llm_context_window`, `llm_request_timeout`, `llm_thinking` (T4.1). Text restructuring: `restructure_page()` called from `extract_page()` via `GlmOcrProvider`; unit + integration tests (T5.4, T5.6, T5.7 — T5.1–T5.3 done in prior commit). Student Dashboard: router registered with four endpoints (`GET /api/student/dashboard`, `GET /api/student/platform-board/{node_id}`, `GET /api/student/home-study/{node_id}`, `GET /api/student/node/{node_id}/topics`); integration tests pass (T6.6, T6.7 — T6.1–T6.5 done in prior commit). 3180 tests, 100% coverage, ruff + mypy clean. `haisir-backend` `feature/rag` at `09aace9`.

> Snapshot baseline: haisir-backend `5925a0ce` (main, hotfix v2026.3.5, 2026-06-13) / `09aace9` (feature/rag, Phase 2 RAG complete, 2026-06-15), haisir-frontend `ad0c923f` (hotfix v2026.3.5, 2026-06-13), haisir-deploy `88cbe5d` (feature/rag, pgvector image wired in compose, 2026-06-14).
> Next session: `git diff 5925a0ce..HEAD` in haisir-backend (main) / `git diff 09aace9..HEAD` in haisir-backend (feature/rag), `git diff ad0c923f..HEAD` in haisir-frontend, `git diff 88cbe5d..HEAD` in haisir-deploy.
