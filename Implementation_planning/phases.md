...
| **G4** — Mastery + post-exam review (refined 2026-06-28) | V37 **adds** `questions.topic_id` (NULLABLE — the 2026-06-24 "already exists" decision was wrong; live code has no such column) + `enrollment_topics` (FK to `student_enrollments(id)`, NOT vision's `enrollments`); `exam_templates.topic_id` NOT added; EnrollmentTopic model/repo + Question.topic_id mapping; MasteryService per-topic recalc (BR-PROGRESS-001/002/003) wired into `submit_exam` completed branch, essay auto-complete, AND manual release/finalize/override path (T4.2.1d); persistence-based `student_at_risk` recovery gate + `topic_marked_weak`; post-exam hAITU review via a new public no-RAG `HaituService` method (`POST /api/haitu/exam-review-chat`, `POST /api/haitu/pattern-analysis` per-worker in-memory cache) + S05 review screen consuming `GET /api/exam-sessions/session/{id}/answers`; weak-topic "Focus areas" strip on the student dashboard | specs, backend, frontend, deploy |
...
Plan doc: `Implementation_planning/PLAN.md` (G4 refined 2026-06-28 — G0–G3 + G2-patch complete; G4 remaining). Decisions: `Implementation_planning/decisions.md` (2026-06-28 G4-refine entry supersedes the 2026-06-24 `questions.topic_id` decision; 2026-06-24 Phase 4 entry). Progress checkboxes: `Implementation_planning/TASKS.md`. Baseline SHAs: backend `9d27e8c`, frontend `23e1a45`, deploy `2ca21d4` (2026-06-28).
