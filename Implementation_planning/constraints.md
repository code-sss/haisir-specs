...
**What:** Coraza is **v3.7.0**, CRS **v4.25.1 LTS**. Blanket `ctl:ruleRemoveById` is down from 38 rule IDs to 1, and that survivor (`931130`) was recorded as structural ("targets a `TX` variable"). **Corrected 2026-09-22:** its primary rule inspects `ARGS`; only the chained sub-rule reads `TX`, so the field-scoped `ARGS_POST` form should apply — pending harness proof (`16_gateway_waf.md` BR-WAF-014).
...
**Impact on target state:** New exclusions must be **field-scoped** (`ctl:ruleRemoveTargetById` with a regex key, or `SecRuleUpdateTargetByTag <tag> "!ARGS_POST:/field/"`); a blanket whole-request removal now needs a written structural justification. More importantly, the durable lesson is upstream of the WAF: **`tx.total_arg_length` is 65535 and rule 920390 is unexcluded**, so a spec that puts unbounded free text or a replayed transcript in a request body hits a ceiling on every route that has not raised it. **Corrected 2026-09-22:** the limit is tunable per route (URI + method scoped `setvar`, precedent `18-api-exam-session-submit.json`; topic-content routes raised by BR-WAF-015), so it is not a hard ceiling — but raising it is a per-route decision sized in **bytes**, not characters. The durable rule stands: cap free-text fields at the schema (`max_length`), size the route's `arg_length`/`total_arg_length` to that cap × 4 bytes, and keep transcripts server-side.
...
## security — a Docker-socket proxy cannot constrain Jenkins, and rootless Docker leaves containers AppArmor-unconfined

**What:** Two facts about the current runtime that repeatedly get mis-specified, both established by the 2026-08-19 full security re-scan (`security/SECURITY_REVIEW_2026-08-19_FULL_RESCAN.md`).

(1) **A scoped socket proxy (`tecnativa/docker-socket-proxy` or equivalent) is not a boundary for the Jenkins agent.** The five pipelines need `docker build` and `docker run`, so the proxy must allow `POST` together with `CONTAINERS`/`IMAGES`/`BUILD` — and `POST` + `CONTAINERS` permits `POST /containers/create`, whose body accepts `HostConfig.Privileged: true` and `Binds: ["/:/host"]`. The proxy is a real boundary only for a consumer needing **no POST at all**, which is exactly CrowdSec (`CONTAINERS=1`, read container list + logs) and exactly not Jenkins. Plain DinD is also excluded: it requires `privileged: true` on the outer container, which `other/security-audit.sh`'s `DOCKER-01` correctly flags CRITICAL.

(2) **Rootless Docker cannot apply an AppArmor profile.** Loading one requires real root, so unlike a rootful deployment every container in this stack runs AppArmor-unconfined; seccomp still applies (the default profile is active, and nothing in the repo sets `seccomp:unconfined`). A container *can* reference a profile already loaded on the host by other means, but nothing loads one today.
