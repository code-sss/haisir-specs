...
- [ ] T5.3.7 [frontend]: Decide whether `src/app/admin/error.tsx`'s inline styles stay — deliberately NOT changed by T5.3.5 (depends on T5.3.5). Its comment states inline styles are intentional so the error boundary renders even if CSS Modules fail during catastrophic chunk errors, which is a sound tradeoff for that path and unlike the spinner's (falsified) justification. It never appeared in the T5.3.2 soak because no admin error was triggered. Under an enforcing CSP it would render unstyled if it ever fires. Needs a decision, not a silent fix.
- [ ] T5.3.8 [frontend]: Narrow `isDevArtifactReport` in `tests/e2e/g5-csp-soak.spec.ts` so the CI soak would have caught T5.3.5 (depends on T5.3.5). The filter discards **all** `style-src-elem`/`style-src-attr` reports as "Next.js dev-tools artifacts"; T5.3.2 proved that assumption false — those same directives fired on the *production* build on staging, and the filter is exactly why they reached prod. Without narrowing it (e.g. to reports whose `source-file` is a dev-tools chunk), the next inline style ships the same way.
- [ ] T5.3.9 [deploy]: Re-run the live OIDC + journey soak against a staging image carrying T5.3.5/T5.3.6, and confirm zero reports (depends on T5.3.5, T5.3.6). **This is what actually closes G5.3.** Sequence: commit the `haisir-frontend` changes → Jenkins frontend build (`TAG=v2026.6-staging`) → deploy → re-run. Do **not** shortcut by pushing a locally built image to `registry.haisir.in`; that skips the pipeline's scan stage and puts an unreproducible artifact on staging. Method that worked for T5.3.2, reuse it: record a UTC marker and the baseline `docker logs haisir-frontend-staging | grep -c csp-report`, have a human drive the real browser round-trip, then diff the count and read the reports back with `jq`. Also confirm the round-trip actually happened in the APISIX access log (`auth/login`, `authenticate`, `auth/callback`, `token`, `logout`) — otherwise a clean result is vacuous.
