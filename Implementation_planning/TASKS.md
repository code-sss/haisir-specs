...
T3.1 (GATE ROOT) passed 2026-07-17 — dev is now fully up (common-project stack + dev/docker-compose.yml + provisioning), so all 8 remaining G3 tasks are unblocked. Recommend T3.7 (plaintext-residue scan) or T3.2 (hash verification) next — both are quick, low-risk checks against the now-live state; T3.4 (OIDC login e2e) and T3.5 (backend-admin no-drift) are the more substantial ones.
