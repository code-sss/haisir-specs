...
> **Last baselined: backend:`ad82740` frontend:`53f573b` deploy:`d36b9e1` (2026-08-10)** — read directly from each sibling repo's HEAD; deploy has uncommitted changes pending (`common/openbao/deploy-required-keys.txt` arms the `KC_HOSTNAME_ADMIN` fail-closed gate; T2.1's pgvector image swap in `common/docker-compose.yml` + `dev/docker-compose.yml`; T2.2's `git rm -r postgres-docker/` (from-source pgvector build + its standalone `haisir-postgres` Jenkins pipeline deleted; `release-manifest` SKILL.md's Postgres-detection path + `.env.template`'s `POSTGRES_IMAGE_TAG` swept off the dead built-image tag); T2.3/T2.4's Postgres uid 70→999 reconciliation + keycloak-db → `reg.mini.dev/postgres:${KEYCLOAK_POSTGRES_IMAGE_TAG}` in `common/docker-compose.yml`; T2.5's APISIX runtime stage → `reg.mini.dev/apache-apisix:3.17.0` in `gateway-docker/Dockerfile`/`Jenkinsfile`/`UPGRADE-RUNBOOK.md`; T2.6's keycloak → `reg.mini.dev/keycloak:${KEYCLOAK_IMAGE_TAG}` + T2.7's etcd → `reg.mini.dev/etcd:${ETCD_IMAGE_TAG}` in both compose files; T2.8's G2 healthcheck → exec-form conversion in both compose files — keycloak's `/dev/tcp` bashism replaced with a `/proc/net/tcp{,6}` LISTEN-grep since the Minimus keycloak JRE image has no shell network client; T2.9 verified Go-builder-digest/go.mod parity holds, no file change) not yet reflected in this SHA.
>
