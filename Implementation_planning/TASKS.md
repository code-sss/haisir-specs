...
- [ ] **G8.1: two independent passes clean** — acceptance test — **NOT CLOSED (2026-08-06).** T8.1.1–T8.1.3 are all done and every one of the 11 distinct findings is fixed or explicitly accepted, but "clean" is not yet earned: three items are owed and are recorded in `security/PHASE7_REVIEW_RESOLUTIONS.md` rather than assumed away. (1) **The image-serving fix has never been exercised live** — it is verified by static reasoning and unit tests only, which is precisely how G3.5 closed originally before shipping a broken feature; its correction must not close the same way. Load an exam question image end-to-end against dev or staging after a route reload. (2) **Watch one behaviour in that test:** `secured-api`'s `openid-connect` may 302 an expired-session `<img>` request to Keycloak, which renders as a broken image rather than a clean 401 — acceptable if confirmed, but confirm it. (3) **The new `WAF Functional Gate` stage has never run** — `waf-harness.sh` needs Docker on the Jenkins agent and binds ports 19080/19180. Separately, decide whether `92a4da2` in the `frontend` devcontainer (a CSP e2e-soak commit that **neither pass covered** — both ran to `d6adec7`) needs review before this closes.
...
- [ ] **G8: Review gate and closeout** — acceptance test — **HARD GATE: no merge until this passes** — **BLOCKED on G8.1 only (2026-08-06).** G8.2 and G8.3 are closed. G8.1 needs the three live verifications above; none require code changes, all require a running stack or a Jenkins run. Everything else in Phase 7 is done: G1–G7 closed, all 11 review findings resolved, specs reconciled, close-out recorded.
