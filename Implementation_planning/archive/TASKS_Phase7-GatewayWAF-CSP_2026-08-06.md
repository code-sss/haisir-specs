...
> **Last baselined: backend:`9d9ea6b` frontend:`d6adec7` deploy:`5aa8524` (2026-08-06, Phase 7 G8
> close-out — read directly from each sibling repo's HEAD, not carried forward).**
>
> **Three caveats on that line, because it is less complete than it looks:**
> 1. **The G8 code changes are in NO baseline.** T8.1.3 (11 review-finding fixes across all three
>    repos) and T8.2.1's Jenkinsfile digest fix are **uncommitted working-tree changes on top of
>    these SHAs**, left that way per this repo's WAF/gateway-edit norm. Re-baseline after they land.
> 2. **The frontend host clone and devcontainer differ.** Host HEAD is `d6adec7`; the `frontend`
>    devcontainer is one commit ahead at **`92a4da2`** ("test(csp): add production CSP enforcement
>    e2e soak"), which is the committed form of the `playwright.prod-csp.config.ts` /
>    `tests/e2e/prod-csp/` files that exist only as untracked files on the host. **Neither G8 review
>    pass covered `92a4da2`** — both ran against the host range ending at `d6adec7`.
> 3. Both devcontainers are **separate checkouts** (named volumes, not bind-mounts), so host edits do
>    not propagate. T8.1.3's changes were applied to both and verified there.
>
> Previous baseline: backend:`e2e0f0f` frontend:`cb930b7` deploy:`ffaab67` (2026-08-05, re-verified
...
> staging matches it. Both acceptances are now CI-enforced in `dev-isolation-check.sh` (no `NET_RAW`/`NET_ADMIN`
...
- [x] **G2.1: multipart charset bypass blocked** — acceptance test (**re-verified and re-closed 2026-08-07 on stronger evidence than the original closure.** The 2026-07-30 closure rested on T2.1.2's engine-level before/after — two throwaway Go harnesses calling `coraza.NewWAF()`/`Transaction` directly, where on Coraza v3.3.3 + CRS v4.14.0 rule 922110 **never appears in `tx.MatchedRules()` at all** and post-upgrade it does. That evidence stands and is the proof the ruleset bump is what fixes the CVE. **What it never covered was the deployed wasm/APISIX HTTP path**, and the live test written for that (`18-test-cve-2026-21876-multipart.sh`) was structurally incapable of reaching rule 922110 — see BR-WAF-013. Running it against staging on 2026-08-07 surfaced this: it reported the payload NOT blocked, because `openid-connect` implements `_M.rewrite` (REWRITE phase) while `coraza-filter` is registered `http_request_phase: "access"`, and rewrite runs before access — plugin priority orders plugins only *within* a phase, so 7999 does not beat 2599 across phases. An unauthenticated `/api/*` request is 401'd before Coraza ever runs. Confirmed live: identical SQLi → **403 on `/`** with 8 rule matches vs **401 on `/api/*`** with **zero** Coraza log lines ever recorded for an `/api/` URI. **Not a protection gap** — authenticated traffic passes rewrite and does reach the WAF, which is why T2.3.2's real browser journeys produced Coraza false positives on `/api/` routes. **Closed by moving the CVE probe into `common/scripts/tests/waf-harness.sh`**, which loads *only* `coraza-filter` (`jq '{plugins: {"coraza-filter": …}}'` strips `openid-connect`, `ua-restriction`, `uri-blocker`), so no other plugin can produce a look-alike 403 and no credentials are needed. **Three probes, because a bare 403 assertion would have repeated the same mistake in a new place:** a multipart *control* (both parts clean UTF-8) that must be proxied **200**, the CVE payload (UTF-7 first part, clean UTF-8 last) that must be **403**, and an assertion that **`id "922110"` actually fired** — the UTF-7 payload decodes to `<script>alert(1)</script>`, so an XSS rule could otherwise block it while 922110 silently stopped working. Verified against the deployed image `registry.haisir.in/haisir-gateway:v2026.6`: **7/7**, with the real log line `Illegal MIME Multipart Header content-type: charset parameter … [id "922110"]` from `REQUEST-922-MULTIPART-ATTACK.conf`. This now runs on **every gateway build** via the `WAF Functional Gate` stage. The standalone `18-test-…sh` was also corrected — it now requires a token and exits non-zero as INCONCLUSIVE without one, so it can never report a silent pass — but it is deploy-time verification, not this gate's evidence.)
...
- [x] T5.3.2 [frontend]: Include the Keycloak OIDC round-trip — `07`/`08`/`09-auth-*` routes are APISIX-owned and redirect cross-origin, exercising `form-action` and navigation (depends on T5.3.1) (2026-08-05; **live soak run twice against staging on `v2026.6`, real browser driven by the user, full round-trip confirmed in the APISIX access log both times — `/auth/login` → `/realms/.../auth` → `login-actions/authenticate` (the actual form POST) → `/auth/callback` → `token` → `userinfo` → `logout`.** **OIDC verdict: CLEAN — zero violations in `form-action`, `frame-src`, `connect-src`, `base-uri`, `object-src`, `frame-ancestors`, `img-src`, `worker-src`, `font-src`.** Two premise corrections, both verified live rather than reasoned from the ticket: (1) Keycloak is **same-origin**, not cross-origin — it is proxied at `{{BASE_URL}}/realms/...`, and its login page carries *Keycloak's own* CSP (`frame-src 'self'; frame-ancestors 'self'; object-src 'none'`) with **no `report-uri`**, so violations on that page are structurally unreportable to our collector and only visible in the browser console; (2) login/logout are `globalThis.location.href` navigations (`landing-page.tsx:13`, `header.tsx:152`, `use-auth.tsx:330`), not form POSTs, so our `form-action 'self'` is never exercised by our own code — the only real form in the flow is Keycloak's, on a page our CSP does not cover. **The soak could not have passed before T5.3.4: `POST /csp-report` was returning 404 and the collector had never received a single report in staging's entire history.** Run 1 (after the route fix) still 403'd all 11 reports at the WAF; run 2 (after the body-access fix) returned 204 on all 7. **G5.3 does NOT close on this task** — run 2 surfaced three real violation classes on the production build, now filed as T5.3.5/T5.3.6/T5.3.7 and fixed but not yet redeployed. T5.4.1 enforcement still MUST NOT proceed per BR-CSP-007 until T5.3.8's post-fix re-run is clean.)
- [x] T5.3.3 [frontend]: Review collected reports and adjust directives (depends on T5.3.2) (2026-07-30; CI soak finding = zero unexplained violations in the scoped directive set, no directive adjustments. Real-production report review incl. the OIDC round-trip is deploy-owned per BR-CSP-007 — T5.4.1 enforcement MUST NOT be unblocked until the deploy soak passes) **AMENDED 2026-08-05 — do not trust this row's "zero unexplained violations" as evidence for prod:** T5.3.2's live soak found `style-src-attr`, `style-src-elem` and `script-src`/`eval` violations on the production build. The CI soak reported zero only because `isDevArtifactReport` discards the `style-src-*` classes wholesale (see T5.3.8) and because no CI journey parses a Zod schema through the JIT path. The real post-fix report review is T5.3.9.
- [x] T5.3.4 [deploy]: Make the CSP report collector reachable in a real environment — `report-uri /csp-report` had no APISIX route and was 404ing (2026-08-05; **found by T5.3.2, this is why every prior "zero violations" reading was vacuous.** `99-catch-all.json` is `methods: [GET, OPTIONS]`, so the browser's `POST /csp-report` matched no route at all; `docker logs haisir-frontend-staging | grep -c csp-report` was **0**. Two fixes, both live on staging and templated: (a) new `common/routes/25-csp-report.json` — `POST`/`OPTIONS` `/csp-report`, priority 10, `secured-anonymous`, → `frontend:3000`, own `limit-count` 100/min/IP. The catch-all was deliberately **not** widened to POST, which would have opened every unmatched path to request bodies. (b) `common/plugin_configs/02-secured-anonymous.json` rule `id:199150`, chained on `REQUEST_URI @rx ^/csp-report$` + `REQUEST_METHOD @streq POST`, placed **before** the CRS include: narrows `tx.allowed_request_content_type` to `|application/csp-report| |application/reports+json|` and sets `ctl:requestBodyAccess=Off`. **Three iterations, the first two proved wrong by live traffic, worth reading before "simplifying" this:** (i) allowing the content type alone — a synthetic probe passed, but all 11 real browser reports 403'd on CRS 920420 before the rule could apply, because 920420 is phase:1 and phase-1 rules run in declaration order, so the rule had to move ahead of the CRS include; (ii) adding `ctl:requestBodyProcessor=JSON` to keep body inspection — 403 again, 9 rules per report (`942120`/`942180`/`942340`/`942370`/`942430` because `original-policy` is the full CSP string with quotes, semicolons and `'nonce-...'`; `941150`/`941170` because `script-sample` carries real script fragments; `931130` because `blocked-uri` is off-domain; `932200`); (iii) `ctl:requestBodyAccess=Off` — 204. A CSP report is by definition a description of attack-shaped content, so body inspection of it can only ever false-positive; per-rule exclusion would have been 18 rules and silent breakage on every CRS bump. **The lesson that generalises: a synthetic WAF probe with a sanitised payload proves nothing** — the first attempt passed only because the probe's `original-policy` was the toy string `"default-src self"`. Scoping re-verified live after the final change: realistic report → 204 and persisted; `application/json`/`x-www-form-urlencoded`/`text/xml` on `/csp-report` → 403 (920420 still enforcing, so this URI is *tighter* than the platform default); `GET /csp-report` → 405; `POST /csp-report?x=1` → 403 (query string does not match `^/csp-report$`, fails closed); `GET /?q=1' OR '1'='1` and `?q=<script>` → 403 (WAF intact elsewhere on the same plugin_config). **Accepted risk, deliberate:** the `/csp-report` body now bypasses CRS inspection. It reaches only `src/app/csp-report/route.ts`, which caps at 64 KiB, `JSON.parse`s inside a try/catch and `console.log(JSON.stringify(...))`s before returning 204 — no DB, no eval, no rendering, no downstream call, and the stringify escaping prevents forging a second log line; volume is bounded by the route's `limit-count` plus `limit-conn`/`limit-req`. Residual: a rate-limited 64 KiB write into the frontend's stdout log. **Uncommitted in `haisir-deploy` as of this note.**)
- [x] T5.3.5 [frontend]: Remove the inline styles behind every `style-src-attr`/`style-src-elem` violation — one loading spinner duplicated across two pages (2026-08-05; found by T5.3.2 run 2: 6 of the 7 reports were this one component. `src/app/page.tsx` and `src/app/home/page.tsx` each rendered a full-screen spinner with two `style="..."` attributes plus `<style>{"@keyframes spin{...}"}</style>` — a nonce can never cover a style attribute, and that `<style>` element was emitted without one while 20 sibling elements got it correctly. Moved to `.auth-loading` / `.auth-loading__spinner` + `@keyframes spin` in `src/app/globals.css`, with a `prefers-reduced-motion` slowdown. The old code comment claimed inline styles were needed because "this renders before any CSS chunk is guaranteed to have loaded" — **checked rather than trusted: the served HTML `<link>`s the global stylesheets in `<head>` ahead of first paint**, so a global class is available exactly as early. That reasoning does still hold for CSS Modules, which is why `src/app/admin/error.tsx` was left alone — see T5.3.7. Verified on a real `next build` + `next start`, not on source: rendered HTML went from 1 nonce-less `<style>` + 2 `style=` attributes to **0 and 0**, and `.auth-loading__spinner{...animation:.8s linear infinite spin}` + `@keyframes spin` are present in the built CSS chunk so the spinner still animates. **Uncommitted in `haisir-frontend`, applied in both the host clone and the `frontend` devcontainer checkout (separate checkouts — the devcontainer mounts a named volume, not a bind-mount).**)
- [x] T5.3.6 [frontend]: Stop Zod v4's JIT compiler from tripping `script-src` as an `eval` violation (2026-08-05; the 7th report from T5.3.2 run 2 was `script-src`/`eval` from a production chunk with **no literal `eval(`** — traced to two `Function()` call sites in `zod@4.4.3`, identified from the bundle's own `f.jitless` flag and `navigator.userAgent.includes("Cloudflare")` probe. Zod v4 JIT-compiles validators via `new Function`. Fixed with `zodConfig({ jitless: true })` at module scope in `src/app/providers.tsx` — a client component evaluated before any schema is first parsed. Zod's typedef documents the flag as "Disable JIT schema compilation. Useful in environments that disallow `eval`", and the API was confirmed by running it (`z.config({jitless:true})` → `z.config().jitless === true`) rather than assumed from docs. **Why this beats adding `'unsafe-eval'` to the production policy, read off the bundled logic rather than guessed:** the emitted code is `if (f.jitless || ...) return !1;` *before* `try{Function("")}`, so with the flag set it short-circuits and **never calls `Function("")`** — no eval attempt and no violation report, instead of relying on the throw-and-fallback path that Report-Only cannot prove works. Verified in the built bundle: `(0,a.config)({jitless:!0})`. Full suite green in the devcontainer — **3939 tests / 221 files passed**, `eslint` and `tsc --noEmit` both clean. **Uncommitted in `haisir-frontend`, applied in both checkouts.**)
- [x] T5.3.7 [frontend]: Decide whether `src/app/admin/error.tsx`'s inline styles stay — deliberately NOT changed by T5.3.5 (depends on T5.3.5) (2026-08-06, frontend `e80a760`; **decision: they go.** The comment's rationale (inline styles so the boundary renders if CSS Modules fail during catastrophic chunk errors) was falsified the same way T5.3.5's was: under the production policy `style-src` carries no `'unsafe-inline'`, so a style attribute is stripped and the boundary renders unstyled *anyway* — the tradeoff bought nothing it claimed to. `globals.css` is `<link>`ed in `<head>` ahead of first paint, and `error.tsx` fires on runtime render errors, not CSS-load failures. Moved to `globals.css`; no resilience lost.)
- [x] T5.3.8 [frontend]: Narrow `isDevArtifactReport` in `tests/e2e/g5-csp-soak.spec.ts` so the CI soak would have caught T5.3.5 (depends on T5.3.5) (2026-08-06, frontend `e80a760`; reports now carry `source-file`, and a style report counts as a dev artifact **only** when sourced from the Next.js dev-tools chunk — app-code style reports fail the soak. Verified both directions: dev-tools noise still filtered, and an injected nonce-less `<style>` now fails the journey. **Narrowing it immediately surfaced real violations the blanket filter had been hiding:** `next/image` emits `style="color:transparent"` on every `<img>`, and `fill` emits a whole position/inset/object-fit block — confirmed present in a real `next build`, not just dev. All 12 call sites already passed `unoptimized` (or are fixed-size local assets), so `next/image` contributed nothing but that inline style; replaced with plain `<img>` + `.img-fill-contain`. Three further static inline styles moved to CSS Modules.)
- [x] T5.3.9 [deploy]: Re-run the live OIDC + journey soak against a staging image carrying T5.3.5/T5.3.6, and confirm zero reports (depends on T5.3.5, T5.3.6) (2026-08-06; **took two runs — run 1 FAILED with two new violation classes, fixed as T5.3.10/T5.3.11, run 2 CLEAN. G5.3 closes on run 2.** Run 1 (image `685c389fbcf4`, frontend `adcd9d3`): 2 reports — `script-src`/`eval` on `/home` and `style-src-attr`/inline on `/courses`. Run 2 (image `407e94ecca9b`, frontend `14a1b47` on top of `e80a760`): **0 reports** beyond a deliberate pre-flight probe, with `/home` and the byte-identical `/courses?source=platform&nodeId=46c850ea-…` URL from run 1 both visited and silent. **Four independent checks that the clean result is not vacuous, all of which have failed on some prior attempt and are worth repeating verbatim next time:** (1) full OIDC round-trip present in the APISIX access log — `/auth/login` → `openid-connect/auth` (200, *and* Keycloak's own CSS/JS 200s, proving the password form actually rendered rather than an SSO cookie short-circuiting it) → `login-actions/authenticate` (302, the real form POST) → `token` → `userinfo` → `/auth/callback` → `/home` → `logout`; **run 1's first attempt had none of these** — the user browsed on a pre-existing session and only 5-minutely `/token` refreshes appeared, which would have made a zero reading meaningless. (2) Both fixes verified **inside the image** before recreating the container — `instrumentation-client` and `jitless:!0` in client chunk `2flkehrwoe-5r.js`, `style-src-attr` in the server chunk — rather than trusted from the `v2026.6-staging` tag, which is mutable and was re-pushed between runs. (3) That same chunk hash observed in the access log, proving the browser loaded the new build and not a cached bundle. (4) Collector proven live by a pre-flight `POST /csp-report` returning **204** — the exact path that was silently 404ing for all of T5.3.2. **Trap worth knowing: `POST /csp-report` from plain `curl` returns 403 from the `ua-restriction` plugin, not the WAF and not a missing route.** It mimics T5.3.4's failure mode exactly; send a browser `User-Agent` or you will chase a phantom. Baselines for the record: marker `2026-08-06T18:28:07Z`, baseline count 1 (the probe), final count 1.)
- [x] T5.3.10 [frontend]: Move Zod's `jitless` opt-out out of `src/app/providers.tsx` into `src/instrumentation-client.ts` — T5.3.6's placement did not take effect (2026-08-06, frontend `14a1b47`; found by T5.3.9 run 1. The `script-src`/`eval` report came from chunk `2lz3j68nqhpwf.js` col 1894; reading the bundled code rather than inferring showed `if (o.globalConfig.jitless || …) return !1; try { return Function(""), !0 }` — it short-circuits when the flag is set, so **reaching `Function("")` proves `globalConfig.jitless` was still false at that moment.** The flag was in the bundle; it just had not been applied yet. Two independent causes, either sufficient: (a) `providers.tsx` is a client component, so module-scope code runs only when *that* chunk evaluates — on the post-OIDC `/home` load a schema was parsed first; (b) `/courses` mounts its own `src/app/courses/providers.tsx` (`CoursesProviders`) which never imports the root providers at all, so on that subtree the config would never run. `instrumentation-client.ts` is Next's designed hook for this — it runs before any other client code, on every route — so one file replaces chasing provider trees. Verified in a real `next build`: `instrumentation-client` + `jitless:!0` land in the same client chunk, loaded ahead of the turbopack runtime.)
- [x] T5.3.11 [frontend]: Add `style-src-attr 'unsafe-inline'` to the policy — the remaining inline styles carry runtime-computed values and cannot be refactored away (2026-08-06, frontend `14a1b47`; decision recorded in `decisions.md` 2026-08-06. T5.3.7/T5.3.8 removed every *static* inline style and explicitly deferred this call; the ~11 survivors are drag-resized panel widths, tree indent depth and `--node-depth`, i.e. JS values that only a style attribute can carry into CSS. A nonce can never cover an attribute. `style-src-attr` overrides `style-src` **for attributes only**, so `<style>` elements and `<link>` stylesheets stay nonce-gated — locked by a unit test in `tests/unit/app/proxy.test.ts` asserting both that the directive is present and that `style-src` is still exactly `'self' 'nonce-n'`, so the relaxation cannot silently widen to elements later.)
...
- [x] T5.4.1 [frontend]: Switch to the enforcing header name, keeping `report-uri` live per BR-CSP-009 (depends on T5.3.3) (2026-08-06; `src/proxy.ts` now emits `Content-Security-Policy` in production and `Content-Security-Policy-Report-Only` in development. Enforce-in-prod / Report-Only-in-dev split is deliberate, not unconditional: the e2e CI soak runs against `pnpm dev` and Next.js dev-tools inject nonce-less `<style>` elements that under Report-Only merely report (filtered as dev artifacts by `isDevArtifactReport`), but under enforcement the nonce nullifies `'unsafe-inline'` and they would be BLOCKED — firing `securitypolicyviolation` events that break the soak's `expect(violations).toHaveLength(0)` and degrading the dev overlay. The codebase already splits directives dev/prod (`isDev`); the header name now follows the same split. This matches G5.3/T5.3.9's note that "green CI means 'no regression', never 'the production policy holds'" — CI is the dev Report-Only regression surface; enforcement is prod, verified by the T5.3.9 live staging soak (zero reports, full OIDC round-trip). `buildCsp` is unchanged — `report-uri /csp-report` retained in both modes (BR-CSP-009). The duplicated `NODE_ENV === "development"` read vs the one inside `buildCsp` is deliberate (`buildCsp` is independently tested/called; no shared helper — YAGNI). **T5.4.2 (dependent, next) is out of scope here**: its "injected inline script is blocked" negative test belongs against a PRODUCTION build (enforcing, no `'unsafe-inline'` in script-src), not dev's permissive policy — the split gives it a real enforcing environment to target rather than blocking it. Unit tests in `tests/unit/app/proxy.test.ts` updated: the `proxy — CSP header` block now stubs `NODE_ENV=production` and asserts `Content-Security-Policy` on public/non-public/redirect paths + a per-request nonce, plus a dev-branch test (`NODE_ENV=development` → Report-Only present, enforcing absent) and a BR-CSP-009 emission-layer test (enforcing header contains `report-uri /csp-report`). `buildCsp`/`frameHostnames`/`mintNonce`/`isPublicPath`/onboarding-guard blocks unchanged. E2e (`tests/e2e/g5-csp-soak.spec.ts` + `helpers/csp.ts`) unchanged — dev stays Report-Only so `expectCspReportOnlyHeader` and all 21 journey tests pass as before. Verified: `pnpm lint` + `pnpm typecheck` + `pnpm test:coverage` (100% statements/branches/functions/lines, 3944 tests / 223 files). **Correction 2026-08-06 (this row previously claimed a pending PR — checked against the actual repo, not assumed):** `d6adec7` is directly on `main`, `git rev-list --left-right --count origin/main...HEAD` reports `0 0` — it is already pushed to `origin/main`, no separate branch exists. Frontend baseline bumped to `d6adec7` in the header banner. **Live-verified on staging 2026-08-06 21:52 UTC:** Jenkins built and pushed a new `v2026.6-staging` image (`3cc22cef91a8`, built `21:46:47Z`, carrying this commit); pulled and the `haisir-frontend-staging` container recreated (`docker compose ... up -d --no-deps frontend`), came up healthy. `curl -A "Mozilla/5.0" https://staging.haisir.in/` confirms exactly one CSP header — the enforcing `Content-Security-Policy`, no `Content-Security-Policy-Report-Only` alongside it — with a real per-request nonce. The "post-enforcement live check on staging" this row flagged as owed is now partially done (header confirmed live); a real login/journey click-through under enforcement is still worth a quick manual pass before treating T7.4.2 as fully safe to build against this.)
- [x] T5.4.2 [frontend]: Negative test — an injected inline script is blocked (depends on T5.4.1) (2026-08-06; the dev e2e suite (`playwright.config.ts`) runs `pnpm dev`, which per T5.4.1's design is always Report-Only — Report-Only never blocks anything, so a real "the browser blocked this" assertion is unwritable against it. Added a **separate** config, `playwright.prod-csp.config.ts`, running against `pnpm build` + `pnpm start` (real `NODE_ENV=production`, real enforcement), plus `tests/e2e/prod-csp/g5-csp-enforce.spec.ts`. Two tests, both passing against a real local production build: (1) the production response carries the enforcing header alone (no Report-Only), with a nonce, and `script-src` has no `'unsafe-inline'` (BR-CSP-002); (2) a script injected into the actual HTTP response body (rewritten via `page.route()` so the browser's real HTML parser inserts it — indistinguishable from a genuine reflected-XSS payload) does not execute, and a real `POST /csp-report` lands reporting `violated-directive: script-src-elem, blocked-uri: inline`. **Methodology finding, not a T5.4.2 bug:** the first draft injected via `page.evaluate()` and the script executed anyway — traced to `page.evaluate()` running through CDP's `Runtime.evaluate`, which Chromium does not gate by page CSP; rewritten to the response-body approach above, which is real enforcement, not a CDP artifact. **Second finding, pre-existing, NOT fixed here (out of scope, already-shipped G5.3 test infra):** `tests/e2e/helpers/csp.ts`'s `attachCspCollector` registers `page.on("securitypolicyviolation", ...)` — confirmed absent from Playwright's actual event types, so it silently never fires; the `violations` array it populates is dead and `assertNoUnexpectedCspViolations`'s `expect(violations).toHaveLength(0)` check has been vacuously true this whole phase. Not a live regression: the soak's real detection is `reportEntries` (the actual `/csp-report` POST interception), which does work and is what T5.4.2 reused. Worth a follow-up cleanup of `csp.ts` at some point. `pnpm exec tsc --noEmit` and `pnpm exec eslint` both clean on the two new files. Left uncommitted for review alongside T5.4.1.)
- [x] **G5.4: CSP enforced** — end-to-end test (2026-08-06; T5.4.1 and T5.4.2 both done — production carries the enforcing header (live-verified on staging, see T5.4.1's note) and a real negative test proves an unnonced injected script is blocked, not just that the header string looks right.)

- [x] **G5: CSP enforced** — end-to-end test (2026-08-06; G5.1–G5.4 all done. G5 closes.)
...
- [x] T6.1.2 [deploy]: Trust the internal CA in the backend image so self-signed Keycloak certs validate rather than being bypassed (depends on T6.1.1) (2026-07-31; deploy doesn't own the backend Dockerfile, so implemented as a runtime trust mount instead of an image rebuild: new external volume `haisir-backend-ca-cert` (holds only `ca.pem`, the same CA that signs Keycloak's cert) mounted read-only at `/certs` in both `backend` and `worker` (`common/docker-compose.yml`), `REQUESTS_CA_BUNDLE`/`SSL_CERT_FILE` env vars point at it. `env-setup.sh` uploads `ca.pem` into the volume, added to `REQUIRED_VOLUMES`. Manual step (user completed live): `export BACKEND_CA_CERT_VOLUME="haisir-backend-ca-cert"` added to dev/staging/prod `.env.config.sh`. T6.1.3/T6.1.4 (backend repo) still needed before this has an effect — trust alone doesn't remove the `CERT_NONE` bypass. **Follow-up gap closed 2026-08-05, found while generating the v2026.6 release manifest:** adding the volume to `env-setup.sh`'s `REQUIRED_VOLUMES` was not sufficient, because `common/scripts/deploy.sh` — the incremental/manifest deployer — keeps its *own* hardcoded volume list in Step 5b and deliberately never calls `env-setup.sh`. A v2026.6 manifest deploy would therefore have failed at `docker compose up` with "volume not found", and merely creating the volume would have been worse than failing: an *empty* `haisir-backend-ca-cert` starts cleanly and then breaks every Keycloak call, since T6.1.1 turned `ssl_verify` on and T6.1.3 confirmed the `CERT_NONE` bypass is gated off. Step 5b now (a) includes `${BACKEND_CA_CERT_VOLUME:-haisir-backend-ca-cert}` in the create loop and (b) seeds `/certs/ca.pem` from `${APISIX_CERTS_VOLUME}`, which already holds the identical internal CA (put there for etcd TLS / T6.3.1), skipping if already populated and hard-failing with an explicit message if neither volume has a `ca.pem`. Verified against real Docker volumes across all three branches (absent → exit 1; source-only → seeded 644 root-owned 1000:1000; already-present → no overwrite), and the escaped remote-script text was rendered to confirm the `${VAR:-default}` expansions reach the remote unexpanded. Standing hazard worth knowing: `env-setup.sh:243` `REQUIRED_VOLUMES` and `deploy.sh` Step 5b are two independent lists over the same `common/docker-compose.yml` volumes — any *future* new external volume must be added to both, or the incremental deploy path breaks exactly this way again.)
...
- [x] T7.4.2 [deploy]: Add the gateway backstop CSP (`frame-ancestors`, `base-uri`, `object-src`, `form-action`) scoped to non-HTML routes only, so it never collides with `proxy.ts`'s policy per BR-CSP-004 (depends on T5.4.1 [frontend]) (2026-08-06; **the real design problem this task existed to solve: the four `secured-*` plugin_configs are each shared between real HTML routes (frontend upstream) and pure API/redirect routes (backend/keycloak upstream, or no upstream at all)** — `secured-api`/`secured-api-upload` are 100% non-HTML (only ever proxy to `backend:8000`), but `secured-anonymous` and `secured-authenticated` are each used by BOTH the frontend catch-all/`home`/`auth-callback` routes AND genuinely non-HTML routes (`/api/auth/csrf`, the Keycloak realm/resource proxies, `/auth/login`, `/auth/logout`). Setting the backstop at the plugin_config level for those two would have set `Content-Security-Policy` on every normal page load too, directly violating BR-CSP-004 (`proxy.ts` already sets a different, nonce-bearing `Content-Security-Policy` there) and likely clobbering it since APISIX's `response-rewrite` `headers.set` overwrites, not merges. **Two-tier fix:** (1) `common/plugin_configs/03-secured-api.json` and `04-secured-api-upload.json` — added the CSP directly at the plugin_config level (unconditionally safe, no route using either ever reaches the frontend). (2) Six individual route files that share `secured-anonymous`/`secured-authenticated` with real HTML routes — `01-keycloak-realms.json`, `02-keycloak-resources.json`, `03-api-csrf.json`, `15-keycloak-admin-resources.json`, `07-auth-login.json`, `08-auth-logout.json` — got a route-level `response-rewrite` override (APISIX route-level plugins fully replace, not merge with, the plugin_config's same-named plugin) carrying the identical existing 7 headers plus the new CSP, so nothing already shipped there regresses. `99-catch-all.json`/`09-auth-callback.json`/`10-home.json` (all frontend-upstream) were deliberately left untouched. `/_next/static/*` and the two headerless Keycloak admin-console routes were deliberately left out of scope — not named by the spec's "direct API responses, error pages" examples, and CSP document-level directives (frame-ancestors/base-uri/object-src/form-action) have no meaning on a non-document asset response. **Live-verified against the dev stack** (all 26 routes + 4 plugin_configs reloaded via `template-configs.sh` + `create_plugin_config.sh`/`create_route_config.sh`, `apisix-dev` restarted): `/api/auth/csrf` (200), `/api/notifications` (401, unauthenticated — backstop still present on the error response), `/realms/haisir-realm-dev/.well-known/openid-configuration` (200), `/auth/login` and `/auth/logout` (302) **all carry exactly `Content-Security-Policy: frame-ancestors 'none'; base-uri 'self'; object-src 'none'; form-action 'self'`**; `/` and `/home` carry **only** `proxy.ts`'s own header (`content-security-policy-report-only` in dev, no gateway CSP alongside it) — zero collision, confirming BR-CSP-004 holds. All 8 edited files `jq`-valid. **Caution for future readers, unrelated to this task's correctness:** immediately after the bulk route+plugin_config reload and one `docker restart apisix-dev`, the gateway briefly gave intermittent 404s on *every* route including ones this task never touched (`/`, `/home`) — a worker-sync settling window, not a config defect; it self-resolved within a few minutes and 30/30 and then repeated large samples all passed cleanly afterward. Worth expecting, not alarming, after any bulk dev-stack reload.)
- [x] **G7.4: header ownership matches the spec table** — integration test (2026-08-06; T7.4.1 and T7.4.2 both done — `X-XSS-Protection: 0` across all four plugin configs, and the backstop CSP now lives only on the non-HTML routes/plugin_configs, live-verified with zero collision against real HTML routes.)
...
- [x] **G7: Residual review items** — integration test (2026-08-06; G7.1–G7.7 all done — G7.4 closes this pass on T7.4.2. G7 closes.)

## G8 [specs]: Review gate and closeout — HARD GATE

### G8.1 [specs]: Independent review passes
- [x] T8.1.1 [specs]: Adversarial security-review pass 1 against the full diff, following the Phase 5.5/5.6 precedent (2026-08-06; `security/PHASE7_SECURITY_REVIEW_PASS1.md` added — full Phase 7 diff reviewed against the plan-baseline SHAs in `PLAN.md` (`deploy 8cb1dbe..5aa8524`, `frontend 3a57718..d6adec7`, `backend 583511d..9d9ea6b`), ~10k human-written lines after excluding the vendored `coraza-proxy-wasm` tree. **9 findings: 2 HIGH, 3 MEDIUM, 4 LOW.** F1 — the new `18-test-cve-2026-21876-multipart.sh` sends its probe with bare `curl` and no `-A`, and `04-secured-api-upload.json`'s `ua-restriction` denylists `curl*`, so a 403 from UA blocking is indistinguishable from a 403 from rule 922110; the script would pass identically against a vulnerable CRS 4.14.0 build, and its `429 → pass` branch treats "rule never evaluated" as success. **The CVE remediation itself is real** — verified at source (CRS `crs_setup_version=4251`, Coraza v3.7.0, 922110 with the fixed regex), only the regression test is worthless. F2 — `src/app/images/questions/[...path]/route.ts` fetches `${NEXT_PUBLIC_BACKEND_URL}/images/...`, which is the **public APISIX ingress** (baked as `https://haisir.in` in `Dockerfile:9`), not `backend:8000`; there is no `/images/*` APISIX route so it falls through `99-catch-all.json` back into the same handler — undici sends no UA, `ua-restriction` `bypass_missing:false` 403s the first hop, and no exam image can render. G3.5 closed on code-level proof with no live round-trip, the unit tests mock `fetch` wholesale, and T2.3.2 saw the symptom on 2026-08-01 and filed it as a devcontainer quirk. F3 — same route's `path.includes("..")` only catches a segment exactly equal to `..`; an embedded `../..` survives and `fetch()` normalizes it, redirecting the cookie-bearing upstream request (latent behind F2). F4 — `exam.py:264` `await file.read()` materialises the whole body before its own 5 MB check, against the 50 MB `SECURITY__MAX_REQUEST_SIZE` this phase set in compose and a 1g backend `mem_limit`; this phase added `read_upload_capped()` for exactly this and wired it into both extraction routes but not the new endpoint. F5 — the WAF binary is built in `reg.mini.dev/go:1.25-dev`, a third-party registry pulled by mutable tag with no digest, and `deploy.sh` builds the CA trust bundle inside tag-pinned `alpine`/`busybox` run as root. F6–F9 low: WebP accepted on upload but rejected by the frontend proxy; the extraction routes' 50 MB cap is unreachable under the 10 MB config default; `worker_processes: 1` vs. the compose comment saying 2; CSRF required on a GET. **Checked and clean** (recorded so pass 2 spends effort elsewhere): server-side grounding + ownership guards on both review-chat endpoints, the `verify_aud`/etcd-TLS/crowdsec-TLS/`sslRequired`/admin-UI/`allow_admin` hardening set, the `/csp-report` 920420 relaxation (correctly chained to POST + `^/csp-report$`, *tighter* than the platform default, before the CRS include, bounded by inherited rate limits and a 64 KiB read-side cap), T7.4.2's six route-level header overrides, the bounded pattern-analysis cache, and `get_safe_filename`. **Explicitly not covered:** the ~29k-line vendored proxy-wasm diff beyond version/patch assertions, the CRS rule files, the tailscale ACL, Jenkins/CI, frontend components, the review_chat SQL, and the test diffs.)
- [x] T8.1.2 [specs]: Adversarial security-review pass 2, independent of pass 1 — 5.6's pass 2 found a live-credential bug pass 1 rated clean (depends on T8.1.1) (2026-08-06; `security/PHASE7_SECURITY_REVIEW_PASS2.md` added. **Genuinely independent**: run by a separate agent on a different model (Sonnet 5), explicitly instructed not to read the pass-1 report or the T8.1.1 row, given the phase goals + an attack-area brief and no pass-1 conclusion. Same diff range as pass 1. **5 findings: 1 CRITICAL, 1 HIGH, 1 MEDIUM, 2 LOW.** **Corroborated independently by both passes** (highest confidence in the phase): the image proxy pointing at the public gateway rather than the backend (F2/P2-1A), `exam.py`'s buffer-before-size-check (F4/P2-3), the unpinned third-party gateway builder (F5/P2-5, severity judgement differs). **Found only by pass 2** — P2-1 Defect B: even with the URL fixed the image proxy forwards only `cookie`, never `Authorization`/`X-Current-Role`, so it can never authenticate against `images.py`'s `current_active_user` dependency; pass 1 traced the URL defect and stopped. P2-2 (the best finding of either pass, squarely the 5.6 defect class): `waf-harness.sh` — the only check that proves the WAF actually filters — is wired into **no** CI pipeline; `gateway-docker/Jenkinsfile`'s "Verify Image" stage runs only `apisix version`, and integration-dast's `*-test-*.sh` glob does not match the filename, so the pipeline can build, scan, SBOM and push a gateway image whose WAF never filters a request with every stage green. P2-4: the 942200 exclusion is plugin-config-wide, not the narrow scope its comment claims. **The passes disagree on one item and pass 2 is wrong**: it listed `18-test-cve-2026-21876-multipart.sh` as a genuine gate; re-verified after the disagreement surfaced and pass 1's F1 stands, with two new facts — (i) ~~the probe URI matches `05-api-write.json`~~ — **this claim was wrong, retracted 2026-08-07**: `16-api-extraction-upload.json` uses `uris` (plural), which the enumeration missed, and matches at priority 20 → `secured-api-upload`, exactly as the file originally said; (ii) `03-secured-api.json`'s `ua-restriction` denylists `curl*` with `rejected_code: 403` and `bypass_missing: false`, and the script sends bare `curl` with no `-A`, so it returns 403 from UA blocking against **any** CRS version. Pass 2's own note that the script *is* picked up by the integration-dast glob makes it worse: it actively produces a green CI signal for a check that cannot fail. **Pass 2 corrected pass 1** on `.secrets.baseline`'s vendored-tree exclusion — pre-commit convenience only, CI Gitleaks scans the tree (`.gitleaks.toml` has no exclusion) and an independent grep of the ~29k lines found nothing; dropped as a finding. **Pass 2 clean list** (recorded for G8.1): CSP set on every proxy.ts code path with correct `strict-dynamic`+nonce, the CSP collector's true streaming cap and its exact-URI WAF relaxation, the admin-surface/etcd/crowdsec/OIDC TLS flips, the tailscale ACL split, `verify_aud`, the five vendored file-level patches verified by direct grep, `_load_owned_completed_session` on every haitu path **including cache hits** with parameterized SQL throughout `review_chat_repository.py`, and the full route-priority set with no collision.)
- [x] T8.1.3 [specs]: Fix or explicitly accept every finding from both passes (depends on T8.1.2) (2026-08-06; `security/PHASE7_REVIEW_RESOLUTIONS.md` added — 14 findings across both passes deduplicated to **11 distinct items** (3 were found independently by both), every one now FIXED, CLOSED-by-another-fix, or ACCEPTED-with-reasoning; nothing left undecided. **CRITICAL P2-1 (both defects) fixed at the root rather than patched:** the cause was that `/images/questions/*` had no APISIX route at all, so it fell through `99-catch-all.json` to the frontend and a Next.js handler tried to do the gateway's job. Added `common/routes/26-images-questions.json` (GET/OPTIONS, priority 20, `secured-api`, upstream `backend:8000`, mirroring `04-api-read.json`) — `secured-api` carries `openid-connect` so **the gateway injects the JWT**, satisfying `current_active_user` with no client-side bearer token per CLAUDE.md's hard rule, which makes Defect B disappear rather than be worked around — and **deleted** `src/app/images/questions/[...path]/route.ts` + its test. Checked before writing the route that image responses are not Coraza body-inspected (`coraza.conf-recommended.conf:91` limits `SecResponseBodyMimeType` to text types), so binary through `secured-api` carries no outbound-anomaly risk. That deletion also **closed F3 and F6** — the weak `path.includes(\"..\")` guard and the PNG/JPEG-only MIME gate both lived in the deleted file; the backend's `_SAFE_FILENAME_RE` is now the single correct guard and already permits the same three formats the upload endpoint accepts. **HIGH P2-2:** new `WAF Functional Gate` stage in `gateway-docker/Jenkinsfile`, after `Verify Image` and **before `Push to Registry`** so a non-filtering image cannot reach the registry. **HIGH F1:** the CVE test now sends a real browser UA (without it `ua-restriction`'s `curl*` denylist returned the same 403 the test read as success), runs a **clean-multipart control probe first** that must return 401 and **exits non-zero as INCONCLUSIVE** if it 403s, no longer treats 429 as a pass, and its comment naming the wrong plugin_config was corrected (**this correction was itself wrong and was reverted 2026-08-07** — `16-api-extraction-upload.json` declares `uris` (PLURAL), which a route enumeration keyed on `uri` misses, and matches the probe at priority 20 with plugin_config `secured-api-upload`; the file's ORIGINAL comment naming `04-secured-api-upload.json` was right). **MEDIUM F4/P2-3:** `exam.py` switched to `read_upload_capped`. **MEDIUM F5/P2-5:** all images digest-pinned — both Dockerfile `FROM`s via new `APISIX_DIGEST`/`GO_BUILDER_DIGEST` args, `deploy.sh`'s four CA-bundle `docker run`s, and `env-setup.sh`'s nine `alpine` uses collapsed to one pinned `ALPINE_IMAGE` var (not named in either finding but the same class, and one is this phase's own CA upload — fixed at the class). **LOW:** `_MAX_UPLOAD_BYTES` now `min(50_000_000, settings.security.max_request_size)` so it cannot claim a limit the stack won't honour; the `mem_limit: 3g` comment corrected (it said `worker_processes` was \"pinned to 2\" and offered dropping to 1 as the next lever, but `config.yaml:159` already sets 1 — that lever was spent). **ACCEPTED:** F9 (CSRF on a GET — strictly stricter than the rule, all callers already use `fetchWithCSRFRetry`, documented in the docstring) and P2-4 (the 942200 `Referer` exclusion is plugin-config-wide; no finer APISIX scoping exists without per-route duplication, which is the treadmill this phase ends — the comment claiming it left 942200 active on \"every other field, header, **and route**\" was wrong and was corrected in all four configs). **Verified:** WAF edits proven **comment-only** by parsing old vs new and diffing the effective directive lists with comments stripped plus every non-Coraza plugin block — unchanged in all four; `jq` valid on all touched JSON; `bash -n`+`shellcheck` clean; `docker build --check` passes with both digests resolving; `ruff` clean; **364 backend tests pass**. Frontend `tsc` (12 errors) and `vitest` (44 failures in `use-exam-authoring.test.ts`) issues confirmed **pre-existing** by reproducing them identically with all changes stashed — none reference the deleted route. All changes left **uncommitted** across the three code repos per the WAF/gateway-edit norm. **Owed before G8.1 closes** (recorded in the resolutions doc, not silently assumed): a live end-to-end image load against dev/staging — G3.5 originally closed on static reasoning alone and shipped a broken feature, so its correction must not close the same way — plus a check of whether `openid-connect` 302s an expired-session `<img>` request to Keycloak, and one real Jenkins run of the new WAF gate.)
- [x] **G8.1: two independent passes clean** — acceptance test (2026-08-07; T8.1.1–T8.1.3 done, all 11 distinct findings fixed or explicitly accepted, and all three owed verifications now discharged against real environments rather than reasoning. **(1) Real image round-trip confirmed by the user on dev** — upload 201 + render, after two further backend fixes they made on top of the lenient-dependency fix (see below). **(2) `WAF Functional Gate` green on a real Jenkins build.** **(3) Staging deploy of v2026.6 verified live:** CSP enforcing on `/` with a per-request nonce and no Report-Only alongside; gateway backstop present on `/api/auth/csrf` and absent on `/` (BR-CSP-004, no collision); CRS 4.25.1 filtering (benign 200 / XSS 403 / SQLi 403); `/images/questions/*` reaching the backend. **The live test earned its keep three times over — every one of these was invisible to static review:** (a) the T8.1.3 digest pins were single-arch manifest digests, not multi-arch index digests, so `alpine/openssl` resolved to linux/arm/v7 and exec-format-errored inside `deploy.sh` Step 5b's CA gate, aborting a staging deploy while wrongly blaming the CA (which was fine); (b) route 26's `desc` was 954 chars against APISIX's 256 limit; (c) **the images fix was only half-done** — pass 2's Defect B named both an `Authorization` JWT *and* an `X-Current-Role` header, and only the JWT half was fixed at the gateway, so every `<img>` render returned `400 X-Current-Role header required`. A browser cannot attach a custom header to an image subresource, so this had no client-side fix; resolved by moving `GET /images/questions/{filename}` to `current_active_user_lenient` as a fourth BR-SEC-006 exemption — a different *kind* of exemption from the three onboarding ones (header not **sendable**, vs. role not yet **known**), recorded as such in `02_auth_and_roles.md` and `CLAUDE.md`. Locked by a regression test that overrides only `verify_token` so the header path actually executes — verified to FAIL against the strict dependency, which the pre-existing tests could not do because they overrode the auth dependency wholesale. **Two further backend fixes by the user, both spec'd:** `Question._normalize_matching_sides()` derives a missing `side` from the canonical L*/R* ID prefix (the authoring UI omits it) — `01_data_model.md`; and a `ValueError` handler mapping domain validation errors to a readable 400 instead of an opaque 500 — `00_overview.md` Key Design Decision 7, which also records the consequence that ~53 `ValueError` messages in `src/` are now user-facing by default. Backend suite 4991 pass.)

### G8.2 [specs]: Runbook
- [x] T8.2.1 [specs]: Write the vendored proxy-wasm upgrade runbook — how to rebase on upstream, re-apply the APISIX patch, bump the version set, and re-run the G2 gate (2026-08-06; `gateway-docker/UPGRADE-RUNBOOK.md` added in `haisir-deploy`, beside `VENDORED.md`/`VERSIONS.md` — placed there rather than in specs following the Phase 5.6 precedent that runbooks live next to what they operate (`common/openbao/rollback-backout-runbook.md`, and 5.6's rotation runbook in `common/openbao/README.md`). `VENDORED.md`'s "Upgrade path" section, which previously pointed at this document only by description, now links it by path and states the division of labour: VENDORED.md records *what* diverges, the runbook records *how* to carry it forward. **Structure:** a "how this component fails" preamble (it fails silently and completely — builds, links, valid `main.wasm`, reports its version, filters nothing; already shipped once); Procedure A (rebase on a newer upstream tag) including the upstream-to-upstream diff technique from T1.3.2 that separates upstream churn from local patches *before* overwriting, the explicit instruction not to resurrect an automated patch script, and CRS-specific steps (the `rules/crs/` tree is byte-identical to stock so the only local customization is the `id:900120` uncomment in `crs-setup.conf.example`; stay on LTS; `fs.go` resolves by directory name so file adds/removals need no code change; read the setup diff for default-on changes like 4.25.1's `/expect/` addition); Procedure B (Go/TinyGo bumps — **`VERSIONS.md` explicitly names T8.2.1 as the owner of this**, since the pins are floors with no BR-WAF-010 failure evidence above them, and there is a standing incentive to try given the 1.46→2.47 GiB memory cost of dropping `nottinygc`) with the four toolchain-disturbable properties and the `tinygo build -x` check that `--stack-first` survived; Procedure C (re-running G2.1/G2.2/G2.3 with real commands, including the `ENV=harness` branch that lets the real suites run without touching secrets, and the warning that the benign-corpus step is the one that cannot be automated — T2.3.2 found three real false positives no synthetic suite caught); a Known Traps section (the silent `default_directives` requirement and its one-line nginx symptom, the memory ceiling with `worker_processes` already spent at 1, digest-pin maintenance); and a Definition of Done checklist. **Loose end from T8.1.3 found and fixed while writing this:** `gateway-docker/Jenkinsfile`'s "Build Gateway Image" stage pre-pulled base images by **tag**, derived from `ARG *_VERSION` — after T8.1.3 digest-pinned the `FROM` lines, that warmed the cache with a potentially different image than the build uses and quietly reintroduced the floating-tag exposure the pinning removes. Now reads `ARG APISIX_DIGEST`/`ARG GO_BUILDER_DIGEST`; both extracted refs verified pullable via `docker manifest inspect`.)
- [x] T8.2.2 [specs]: Document the CRS upgrade cadence and where the LTS track is tracked (depends on T8.2.1) (2026-08-06; new "CRS upgrade cadence and where the LTS track is tracked" section in `gateway-docker/UPGRADE-RUNBOOK.md`, plus `BR-WAF-003` in `target/requirements/16_gateway_waf.md` rewritten to carry the identification rule and point at it. **Everything read from the upstream GitHub API on 2026-08-06, not asserted** — both documented queries were then re-run verbatim to confirm they work copy-paste. **The finding that justifies the task: LTS is marked in the GitHub release *name* (`v4.25.1 (LTS)`), never in the tag, the version number, a branch, or any API field.** `v4.28.0` and `v4.25.1` look equally current; only the name distinguishes them, so the obvious "upgrade to latest" move silently leaves the supported track — the spec previously said "prefer the LTS track" without saying how to tell. **Observed cadence** (from the real release history, not upstream's stated policy): rolling ~monthly in the first days of the month (v4.19.0→v4.28.0, ten releases Oct 2025–Jul 2026); LTS patches roughly quarterly (v4.25.0 2026-03-28 → v4.25.1 2026-07-02) and **entirely backports** — every v4.25.1 entry reads "cherry-pick of #NNNN"; release days coordinated across tracks (v4.25.1/v4.28.0/v3.3.10 all 2026-07-02; v4.25.0/v3.3.9 both 2026-03-28), so a security fix lands on every supported line the same day; advisories publish ~1 day after the fix. **Recommended cadence: quarterly + on any CRS advisory** — monthly checking buys nothing on an LTS track, so the advisory feed is the real trigger and the quarterly check the backstop. **Current position verified against every open advisory: we ship 4.25.1, which is the newest LTS, and are patched on all four** — including the two published 2026-07-03 (GHSA-f5qm-3h4p-8qhg ReDoS, GHSA-6jp8-c2w2-x7wr XML attribute bypass), both listing the LTS branch affected `<= 4.25.0`; on the rolling track the same fixes needed 4.28.0, which is the concrete vindication of choosing LTS. Also documented: **how to read CRS release notes from a Coraza deployment** — they are written for ModSecurity v2/v3, so v4.25.1's "Breaking Changes" (requiring libmodsecurity ≥3.0.16 for rule 901181's `XML://@*` ctl action) does not bind us at all, while the converse risk is that a change upstream deems benign on ModSecurity behaves differently on Coraza, which only `waf-harness.sh` + the G2.3 corpus settles. Flagged for the next bump: v4.25.1 backported a **942200** false-positive fix for "commas but no whitespace" (the exact shape behind this repo's `selected_nodes` exclusion) and a **930120** fix for parameter names containing `.history` (this platform sends a `history` field) — but the 942200 false positives were still observed 2026-08-01 against an image *already carrying 4.25.1*, so the backport does not cover our case; recorded as "re-test whether the local exclusion is still needed" rather than asserting it either way.)
- [x] **G8.2: the next upgrade does not require rediscovery** — acceptance test (2026-08-06; T8.2.1 and T8.2.2 both done. `haisir-deploy/gateway-docker/UPGRADE-RUNBOOK.md` carries the rebase procedure, the ten divergences to carry forward, the Go/TinyGo bump trial this project still owes under BR-WAF-010, the G2.1/G2.2/G2.3 re-run steps with real commands, the known traps, a done-checklist, and the CRS cadence + LTS-identification rules. `VENDORED.md` links it by path. Criterion met on the specific test that the phase's own rediscovery cost was the *silent* failures — the runbook leads with them: the LTS-in-release-name rule, the fact that a green build and a 4/4 harness both prove nothing about ruleset version, and the `default_directives` omission that disables the WAF entirely with one easily-missed warn line.)

### G8.3 [specs]: Reconcile specs with reality
- [x] T8.3.1 [specs]: Update `16_gateway_waf.md` and `15_security_headers.md` status notes to what actually shipped (2026-08-06; a **SHIPPED** status box added at the top of each, with a scoping-vs-shipped comparison table, and the original 2026-07-27 scoping notes retained beneath for context rather than overwritten. Figures measured against the tree, not copied from task notes: blanket `ctl:ruleRemoveById` **38 -> 1**, and the survivor `931130` recorded as a *structural* exception (it targets a `TX` variable, so the `ARGS_POST` regex form that replaced the other 37 cannot apply) rather than left looking like residue; 41 `ctl:ruleRemoveTargetById` + 15 `SecRuleUpdateTargetById` + 5 `...ByTag` field-scoped forms now in use; Coraza v3.3.3->v3.7.0, CRS v4.14.0->v4.25.1 LTS. `16_gateway_waf.md` also records the ten vendored divergences, the build-time assertion of all ten, the `WAF Functional Gate` now blocking before registry push, the silent-failure property that motivates both, and the operational ceiling (`worker_processes` 1, `mem_limit` 3g on a ~4 GB host; `nottinygc` removal cost 1.46->2.47 GiB). `15_security_headers.md` records enforcement, the render-side/gateway split holding under implementation, the two premise corrections the live OIDC soak forced (Keycloak is same-origin and carries its own `report-uri`-less CSP; login/logout are `location.href` navigations so our `form-action` is never exercised by our own code), both documented relaxations, and 0 statically-prerendered routes re-verified after the review removed one.)
- [x] T8.3.2 [specs]: Update `security/SECURITY_REVIEW_2026-07-02.md` annotations to final status (depends on T8.3.1) (2026-08-06; the two rows still open at the 2026-08-04 refresh both closed today and are now recorded per-finding and in the summary table: **M1** (CSP enforcing in production, live-verified on staging, with the 12 previously-prerendered pages opted into dynamic rendering and re-verified at 0 static routes) and **L3's G7.4 dependency** (T7.4.2's backstop CSP scoped to non-HTML routes, live-verified present on the API/Keycloak/auth routes and absent on `/` and `/home`). Added a **FINAL** closeout block stating that no row remains open; that **four findings were closed by deciding not to fix them** (L5, M6/L4, M5's Postgres-TLS residual, the Admin API static key) each with a written record rather than a silent pass; and that **this review's own M1 recommendation was wrong** — a nonce CSP at the gateway is not implementable, since APISIX cannot mint a per-request value and inject it into the rendered body — recorded rather than quietly dropped. The block also scopes the document honestly: it is a 2026-07-02 artifact, Phase 7 surfaced findings after it, and Phase 7's own diff was reviewed separately in `PHASE7_SECURITY_REVIEW_PASS1/2.md` + `PHASE7_REVIEW_RESOLUTIONS.md`. Disposition per the file's own Lifecycle note is to archive it — deliberately **not** done yet, because `target/requirements/14_container_images.md` still cites it for Phase 8 container-UID context; move it when that citation is repointed.)
- [x] T8.3.3 [specs]: Append the Phase 7 close-out entry to `decisions.md` and `progress.md` (depends on T8.3.2) (2026-08-06; **`decisions.md`** — new top entry (newest-first, append-only respected) recording the phase-level outcome and the decisions taken at closeout itself: the version-gap root cause and why fixing it dissolved the treadmill; keeping the vendored tree and deleting the pattern-matching awk patch script; the CSP nonce belonging at the renderer with the gateway carrying only a non-colliding backstop; enforcement gated on live evidence and the two premises that evidence corrected; and the four deliberate non-fixes. It also records what G8's two passes cost and returned — that independence paid **in both directions** (pass 2 found the un-wired WAF gate, pass 1 found two vendored-tree defects pass 2 read past, and the passes disagreed once with pass 2 wrong) and that **the recurring defect class this phase was false assurance rather than missing controls**, in three separate instances. **`progress.md`** — the Phase 7 Target State paragraph flipped from "spec-only, not yet implemented / implementation starting" to **SHIPPED, closed 2026-08-06**, and a new Current State section added covering WAF, gateway build, CSP, the review closeout, the application-side fixes that removed the false-positive source, and the G8 gate. Both entries end with what is **owed rather than claimed**: a live end-to-end load of the rebuilt image path (the fix is static/unit-verified only, and G3.5 closed on exactly that basis and shipped a broken feature), one real Jenkins run of the new WAF gate, and a decision on `92a4da2` — a CSP e2e-soak commit in the frontend devcontainer that neither review pass covered.)
- [x] **G8.3: specs match the shipped system** — acceptance test (2026-08-06; T8.3.1–T8.3.3 all done. `16_gateway_waf.md` and `15_security_headers.md` carry SHIPPED status boxes with scoping-vs-shipped tables measured against the tree rather than the task log; `SECURITY_REVIEW_2026-07-02.md` has no open row and a FINAL closeout block that also records where its own M1 recommendation was wrong; `decisions.md` and `progress.md` carry the phase close-out. Criterion met in the strict direction too — the numbers in the specs were re-derived from the configs at closeout, so they describe what shipped rather than what was planned.)

- [x] **G8: Review gate and closeout** — acceptance test — **HARD GATE: PASSED 2026-08-07.** G8.1, G8.2 and G8.3 all closed. Two independent adversarial passes with every finding resolved; the vendored proxy-wasm upgrade runbook and CRS LTS cadence documented so the next upgrade needs no rediscovery; and the specs reconciled to what actually shipped. Phase 7 closes. **Both carried-forward items are now closed too (2026-08-07):** (1) the `92a4da2` CSP e2e-soak question is resolved — investigation found it was **worse than 'un-reviewed'**: `playwright.config.ts` carries `testIgnore: ["**/prod-csp/**"]` and Jenkins runs the default config, so the soak had been committed and was executing **nowhere**; a green pipeline said nothing about it. This is the *third* instance of the phase's dominant defect class (after the CVE test that could not fail and the un-wired WAF harness). Fixed by a new `CSP Enforcement Soak (prod build)` stage in the frontend `Jenkinsfile` (frontend `705833d`), placed after `Build Next.js` — the config's webServer runs `pnpm start` and deliberately does not build — and before `Build Docker Image`/`Push to Registry`, so a failing soak cannot ship. Verified locally (2/2 pass against a real production build, the report showing `disposition: enforce` / `blocked-uri: inline`, i.e. the browser genuinely refused an unnonced script) and then **confirmed green on a real Jenkins run**. (2) The Postgres collation-version mismatch — unrelated to Phase 7 but surfaced by its deploy verification — is fixed on **both** environments (2.42 → 2.43, 0 invalid indexes each) and recorded as backlog B2 in `phases.md`, alongside B1 for the worker `idle in transaction` sessions that blocked the first reindex attempt and remain unfixed.
...
> **G1, G2, G3, G4, G6 are all now closed.**
> **T5.4.1 AND T5.4.2 both done — G5.4 and G5 CLOSE.** (Corrected below: the note that originally
> stood here had a broken blockquote line and two stale claims — that T5.4.1 was uncommitted on a
> pending-PR branch, and that T5.4.2 was still open. Neither is true; see each task's own note in
> the G5.4 section above for the full evidence, summarized here.)
> `src/proxy.ts` now emits the enforcing `Content-Security-Policy` header in production
> (Report-Only retained in development as the CI regression surface). `d6adec7` is directly on
> `main`, already equal to `origin/main` — not on a branch awaiting a PR. **Live-verified on
> staging, not just locally:** Jenkins built and pushed a new `v2026.6-staging` image
> (`3cc22cef91a8`), pulled and `haisir-frontend-staging` recreated, came up healthy;
> `curl -A "Mozilla/5.0" https://staging.haisir.in/` confirms exactly one CSP header — the
> enforcing one, nonce present, no `Content-Security-Policy-Report-Only` alongside it.
> T5.4.2's negative test runs against a real local production build (`pnpm build` + `pnpm start`,
> new `playwright.prod-csp.config.ts` — the existing e2e config runs `pnpm dev`, which per T5.4.1's
> design is always Report-Only and can never prove a block) and passes: an inline script written
> into the real HTTP response body (not `page.evaluate()` — that runs through CDP and bypasses
> page CSP, a real methodology trap this session hit and fixed) does not execute, and a real
> `POST /csp-report` reports `script-src-elem`/`blocked-uri: inline`.
> **T7.4.2 done — G7.4 and G7 CLOSE. G1–G7 are now ALL closed.** User confirmed the staging login
> click-through above (2026-08-06: "I did login to staging and I can login successfully and see
> home page") — the last "still worth doing" item from the previous note. See T7.4.2's own note for
> the full design (the four `secured-*` plugin_configs are each shared between HTML and non-HTML
> routes, so the backstop needed per-route/per-plugin_config scoping, not a blanket add) and the
> live-verification evidence (backstop CSP confirmed present on `/api/auth/csrf`, `/api/notifications`,
> the Keycloak realm proxy, and `/auth/login`/`/auth/logout`; confirmed absent — zero collision — on
> `/` and `/home`, which still carry only `proxy.ts`'s own header).
> **Only G8 remains — the hard review/closeout gate, entirely specs-owned.** Ready now (no
> dependencies): **T8.1.1** (adversarial security-review pass 1 against the full diff), **T8.2.1**
> (vendored proxy-wasm upgrade runbook), **T8.3.1** (update `16_gateway_waf.md`/`15_security_headers.md`
> status notes). This is real review/documentation work, not mechanical — worth confirming scope
> with the user before starting rather than assuming which to pick up first.
> **Baselines:** frontend `origin/main` is `d6adec7` (bumped from `e94ff33` — includes T5.4.1).
> Deploy unchanged at `fc24e6f`, plus this pass's 8 uncommitted route/plugin_config edits for
> T7.4.2 (left for review, per the established WAF/CSP-edit review norm). Staging frontend image is
> now `3cc22cef91a8` (was `407e94ecca9b`).
>
...
> **Ready now — one item, and it is again the whole critical path:**
> 1. **T5.4.1** [frontend] — switch `Content-Security-Policy-Report-Only` to the enforcing header
>    name in `src/proxy.ts`, keeping `report-uri` live per BR-CSP-009. Its dependency (T5.3.3) was
>    already done; the real blocker was BR-CSP-007's "do not enforce until the soak passes", which
>    T5.3.9 has now satisfied. Everything left in the phase sits behind it: T5.4.2 + T7.4.2 →
>    G5.4/G5 + G7.4/G7 → **G8**.
> **Read T5.3.9's and G5.3's notes before starting T5.4.1** — two limits on the evidence bear
> directly on it: the live soak only covers routes actually walked, and the dev CI soak
> *structurally cannot* reproduce either T5.3.9 finding (dev `script-src` carries `'unsafe-eval'`,
> which a nonce does not nullify). Plan for at least one post-enforcement live check rather than
> treating green CI as proof.
> **Baselines bumped:** frontend `origin/main` is **`e94ff33`** (Phase 7 CSP work is `e80a760`
> T5.3.7/T5.3.8 + `14a1b47` T5.3.10/T5.3.11; `e94ff33` is unrelated alt-text wording). Staging runs
> frontend image **`407e94ecca9b`** (`v2026.6-staging`), container recreated 2026-08-06 18:27,
> healthy, CSP still Report-Only. `haisir-deploy` unchanged at `fc24e6f`; its `25-csp-report.json`
> collector route and rule `id:199150` are committed, so the earlier "a clean redeploy would revert
> the collector fix" caution no longer applies.
> **G3.2 and G3.4 remain Ready now, unchanged** — both still need a live-backend round-trip and
> were not touched this pass. With a healthy staging stack up they are cheap to close in one
> session alongside T5.4.1.
> **Known, deliberately not acted on:**
> - Staging `/api/student/dashboard` and `/api/student/nodes?owner_type=platform` return **422**
>   after login. **Expected — this is the instructor persona, whose new design is not finished.
>   Do not investigate as a regression** (user confirmed 2026-08-06).
...
> with a real browser. Read T5.3.2's and T5.3.4's notes before touching CSP again; the short version:
> **the soak had never been capable of passing.** `report-uri /csp-report` had no APISIX route (the
> catch-all is `GET`/`OPTIONS` only), so every violation report 404'd and staging's frontend log had
> received **zero** reports, ever. Fixing that (T5.3.4) then took three WAF iterations, two of which
> a synthetic probe wrongly certified as working.
> **The OIDC question T5.3.2 was chartered to answer is settled: CLEAN** — no `form-action`,
> `frame-src`, `connect-src`, `base-uri`, `object-src` or `frame-ancestors` violations across the
> full round-trip. Two premise corrections recorded on the task: Keycloak is **same-origin** here,
> not cross-origin, and its login page carries its own `report-uri`-less CSP; and login/logout are
> `location.href` navigations, not form POSTs.
> **G5.3 still does NOT close, and T5.4.1 still MUST NOT proceed per BR-CSP-007.** The soak found
> three real violation classes on the *production* build — a duplicated inline-styled spinner
> (T5.3.5) and Zod v4's JIT `new Function` (T5.3.6). Both are **fixed and verified against a real
> `next build`, but uncommitted and not yet deployed**, so the post-fix re-run has not happened.
> **Ready now — one item, and it is the whole critical path:**
> 1. **T5.3.9** [deploy] — re-run the soak once staging carries T5.3.5/T5.3.6. Blocked only on a
>    human: commit `haisir-frontend`, run the Jenkins frontend build, deploy. Everything from
>    T5.4.1 → T5.4.2 + T7.4.2 → G5.4/G5 + G7.4/G7 → G8 sits behind it.
> **Also newly open, neither blocking T5.3.9:** **T5.3.7** [frontend] (decide `admin/error.tsx`'s
> inline styles — deliberately not touched, its CSS-Modules-may-fail rationale is sound unlike the
> spinner's) and **T5.3.8** [frontend] (narrow `isDevArtifactReport`, the CI filter that discarded
> `style-src-*` as dev-only artifacts and is exactly why T5.3.5's violations reached production).
> **G3.2 and G3.4 remain Ready now, unchanged** — both still need the live-backend round-trip; this
> session did not get to them.
> **Baselines unchanged — `haisir-deploy` and `haisir-frontend` both have UNCOMMITTED work** from
> this session: deploy = `common/routes/25-csp-report.json` (new) + `common/plugin_configs/02-secured-anonymous.json`
> (rule `id:199150`); frontend = `src/app/globals.css`, `src/app/page.tsx`, `src/app/home/page.tsx`,
> `src/app/providers.tsx`. The deploy pair is **already loaded live on staging** (routes and all four
> plugin_configs reloaded, `.templated/` regenerated), so staging's running config is ahead of
> `origin/main` until those land — a redeploy from a clean checkout would revert the collector fix.
> **Operator item, unrelated to CSP, found in passing:** staging's `APISIX_ADMIN_URL` is still pinned
> to the Tailscale IP `9180` while T7.7.2 correctly moved the admin port to `127.0.0.1` only
> (`docker port apisix-staging` confirms). `deploy.sh` overrides the URL so deploys work, but any
> **standalone** script invocation fails with curl exit 7 — the same trap T7.7.2's note flagged for
> prod's certbot renewal hook, live on staging now. Needs a `.env.config.sh` edit by the user.
