...
9. ~~**Search embedding model**: `00_overview.md` search convention specifies hybrid search using `pgvector` but the embedding model was listed as "Default TBD."~~ **Resolved:** `all-MiniLM-L6-v2` (sentence-transformers, self-hosted). Chosen for low infrastructure cost and zero API dependency in v1. 384-dimension vectors; model served via a lightweight sidecar container. Swap to a multilingual model post-launch if Hindi search quality proves insufficient. See `00_overview.md` search convention.
