...
| M6 — etcd auth disabled | **FIXED for prod/staging** — dev-only pattern, correctly isolated | G7.5 (reframe) |
...
| L4 — dev conveniences | **FIXED for prod** — reframe as a regression guard | G7.5 |
| L5 — `referer-restriction bypass_missing` | **OPEN** — accepted as a spam filter, not a boundary | G7.5 (document) |
...
> **[2026-07-27] STILL OPEN.** `openid-connect.ssl_verify: false` at `03-secured-api.json:407`, `01-secured-authenticated.json:305`, `04-secured-api-upload.json:308`; `etcd.tls.verify: false` at `common/apisix_conf/config.yaml:48`; CrowdSec `ssl_verify: false` at `:72` — moot in practice because `crowdsec_lapi_scheme` is `"http"` (`:67`), so the bouncer key traverses the Docker network in plaintext. Also newly found: OpenBao's own database secrets engine connects with `sslmode=disable` (`common/openbao/bootstrap.sh:252`). Phase 7 G6.3.
...
> **[2026-07-27] FIXED for prod/staging; dev pattern intact and correctly isolated.** Prod/staging run `common/docker-compose.yml`, where etcd uses `https://0.0.0.0:2379` with `ETCD_CLIENT_CERT_AUTH=true` and cert/key/CA files (`:496-502`), runs as uid 1000, `read_only: true`, `cap_drop: ALL`, and **publishes no host port**. `ALLOW_NONE_AUTHENTICATION=yes` survives only in `dev/docker-compose.yml:85`, whose etcd is also unpublished. Reframed in Phase 7 G7.5 as a **regression guard** (CI assertion that dev-only patterns never appear outside `dev/`) rather than a live finding. Residual: `etcd.tls.verify: false` means APISIX presents a client cert but does not verify etcd's server cert — that part is M5, still open.
...
> **[2026-07-27] FIXED for prod.** `common/docker-compose.yml`: Keycloak runs `command: [start]` (`:416-417`), `KC_HOSTNAME_STRICT=true` (`:415`), `KC_HTTP_ENABLED=false` (`:409`); Keycloak admin (`:424`), APISIX admin (`:561`) and DB (`prod/.env:57`) port bindings all resolve to the Tailscale IP in prod; **no pgAdmin service exists in the prod compose at all**. Dev conveniences are confined to `dev/docker-compose.yml`. Reframed in Phase 7 G7.5 as a regression guard rather than a live finding. Residual: `dev/.env.config.sh:54` sets `KEYCLOAK_ADMIN_ALLOWED_CIDR="0.0.0.0/0"` — dev only; prod is `""` and staging is a `/32`.
...
> **[2026-07-27] STILL OPEN, and this review's characterisation is endorsed.** Present in seven files (`01`/`02`/`03`/`04` plugin configs, `01-keycloak-realms.json`, `13-keycloak-admin.json`, `14-keycloak-master-realm.json`). Phase 7 G7.5 **documents this as an accepted spam filter rather than fixing it** — it is correctly backed by CSRF tokens, OIDC and IP allowlists on sensitive routes.
