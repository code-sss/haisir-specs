...
sequenceDiagram
    ...
    U->>FE: Click "Become a tutor"
    U->>FE: Fill profile (subjects, grades, bio, marketplace toggle)
    FE->>BE: POST /api/users/me/become-tutor
    BE->>K: Assign "tutor" realm role (Admin API)
    K-->>BE: OK
    BE->>BE: Create tutor profile in DB (one transaction)
    BE-->>FE: {assigned: true, role: "tutor", profile_id}
    FE->>K: Silent re-auth (hidden iframe, prompt=none)
    K-->>FE: New JWT — realm_access.roles includes "tutor"
    FE->>BE: GET /api/users/me
    BE-->>FE: Updated user with tutor role
    FE->>FE: Add "Tutor" pill to role switcher
    Note over FE: If marketplace_listed=true, profile is live immediately
```
...
sequenceDiagram
    ...
    participant BE as Backend
    participant K as Keycloak
    participant I as Instructor (new user)

    IA->>BE: POST /api/admin/invite-role<br/>{email_or_sub, role: "instructor", organization_id}
    BE->>BE: Verify caller is institution_admin<br/>Verify org ownership
    BE->>K: Create / find user account (Admin API)
    K-->>I: Send invite email (Keycloak-managed)
    I->>K: Accept invite — set password
    BE->>K: Assign "instructor" realm role (Admin API)
    BE->>BE: INSERT org_memberships record in DB

    Note over I,BE: First login to /teacher
    I->>BE: GET /teacher
    BE-->>I: Inline profile setup prompt
    I->>BE: POST /api/teachers/me/profile<br/>{subjects, grades, years_experience}
    BE-->>I: {profile_id} — teacher workspace ready
```
...
**Invite-link enrollment:** When a student arrives at the onboarding flow via a CSV invite link generated by `POST /api/organizations/{org_id}/enroll-students/csv` (BR-INST-017), the invite URL carries a pre-populated enrollment context (`class_id`, `org_id`, `grade`, `section`). The onboarding flow must detect this parameter and, after the student completes sign-up and role assignment, auto-enroll them into the specified class without requiring the student to enter an invite code manually.
