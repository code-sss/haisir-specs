...
**BR-NOTIF-007:** `assessment_due_soon` is generated by a scheduled job (cron) running every hour in UTC, looking for assignments where `due_at` is between now and now+24hrs and no `assessment_due_soon` notification has already been sent for that assignment+student combination.
