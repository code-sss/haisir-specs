...
>
> **Migration complete.** Existing assessment data has been migrated to `exam_templates` with `mode = 'static'` and `purpose = 'quiz'`. No further migration work is needed.
...
class ExamTemplate(BaseModel):
    id: UUID
    course_path_node_id: UUID   # FK → course_path_nodes
    topic_id: UUID | None       # FK → topics — required when purpose = 'quiz', null when purpose = 'exam'
    title: str
    description: str | None
...
**BR-EXAM-PURPOSE-001:** `purpose = 'quiz'` templates are formative — tied to a single topic (`topic_id` required, not null). Students can take them anytime via the topic navigator, which looks up available quizzes by `topic_id`. `purpose = 'exam'` templates are summative — can span multiple topics (`topic_id` is null). Students access them only when assigned. Both types can be assigned to a class via `class_assignments`.
...
ALTER TABLE exam_templates
  ADD COLUMN topic_id UUID NULL REFERENCES topics(id),  -- required for purpose = 'quiz', null for purpose = 'exam'
  ADD COLUMN owner_type VARCHAR(20) NOT NULL DEFAULT 'platform'
    CHECK (owner_type IN ('platform', 'institution', 'tutor')),
  ADD COLUMN organization_id UUID NULL;  -- FK → organizations (new table)
```

**`topic_id` backfill:** All existing `exam_templates` rows have `purpose = 'quiz'` (migrated from `assessments` in Phase 0). Before enforcing NOT NULL on quiz rows, a one-time backfill is required to set `topic_id` on these rows. Queries that return templates for the topic navigator must filter by `topic_id = :topic_id WHERE purpose = 'quiz'`.

> **Unified with `course_path_nodes` and `topics`** — all three tables use the same `('platform', 'institution', 'tutor')` enum for `owner_type`. Instructors who create exam templates within an institution context get `owner_type = 'institution'` and `organization_id = their_org_id`. There is no separate `instructor` ownership; instructors author on behalf of their institution.

**Backfill:** Set all existing `exam_templates` rows to `owner_type = 'platform'`, `organization_id = NULL`.

**BR-EXAM-OWNER-001:** `owner_type` and `organization_id` for `exam_templates` are set at creation based on the creator's role context:
- Creator is `admin` → `owner_type = 'platform'`, `organization_id = NULL`
- Creator is `institution_admin` or `instructor` → `owner_type = 'institution'`, `organization_id = org_id`
- Creator is `tutor` → `owner_type = 'tutor'`, `organization_id = NULL`

**BR-EXAM-OWNER-002 (Board adoption cloning):** When an institution adopts a platform board (via `board_adoptions`), all `exam_templates` associated with that board's `course_path_nodes` are cloned as institution-owned copies (`owner_type = 'institution'`, `organization_id = org_id`). The platform originals remain unchanged. This allows institutions to customize adopted exam templates without affecting the platform source.
...
Two tables manage the parent-child linking flow. `student_link_codes` holds the currently active code for each student (single source of truth). `parent_child_links` records each established parent–student relationship.

**`student_link_codes`** ❌ NEW — one row per student, replaced in-place when a new code is generated.

```python
class StudentLinkCode(BaseModel):
    student_idp_sub: str    # PK — no FK constraint
    code: str               # 6-char alphanumeric uppercase, unique platform-wide
    expires_at: datetime    # generated_at + 7 days
    created_at: datetime
```

**`parent_child_links`** ❌ NEW — one row per (parent, student) pair. Created when a parent successfully claims a code.
...
class ParentChildLink(BaseModel):
    ...
    parent_idp_sub: str
    student_idp_sub: str
    linked_via_code: str    # the code used to create this link — audit only, not used for validation
    linked_at: datetime
    status: Literal['linked', 'revoked']
```

**BR-PARENT-001:** One active link code per student at a time — enforced by `student_link_codes.student_idp_sub` as PK (upsert replaces the previous code). Multiple parents can claim the same code before it expires — each claim creates a new `parent_child_links` row.
**BR-PARENT-002:** Link code expires 7 days after generation (`expires_at`). The validation endpoint checks `expires_at` and returns 410 if expired.
**BR-PARENT-003:** A parent can link to multiple children. A child can have multiple linked parents. No hard limit on the number of children per parent in v1.
**BR-PARENT-004 (Link code format):** Link codes are 6-character alphanumeric uppercase strings (e.g., `A3K7M2`). Generated server-side using cryptographically secure random (`secrets.token_hex` or equivalent). Unique constraint on `student_link_codes.code` prevents collisions.
**BR-PARENT-005 (Rate limiting):** Link code validation endpoint (`GET /api/parent-link-codes/{code}`) enforces a maximum of 10 validation attempts per IP per hour. Exceeding this returns 429.
**BR-PARENT-006 (Revocation):** Either party can revoke the link at any time. Student revokes from their profile (screen S10). Parent revokes from the parent dashboard. Revocation is immediate — `status` changes to `'revoked'` and the parent loses access on their next API call (enforced by BR-SEC-004). Revocation cannot be undone; a new link code must be generated to re-link.
...
-- Student link codes
CREATE UNIQUE INDEX idx_student_link_codes_code ON student_link_codes(code);
-- (student_idp_sub is PK — no additional index needed)
...
Hybrid search uses PostgreSQL full-text (`tsvector` + GIN) combined with `pgvector` semantic embeddings. The following columns and indexes are added to searchable entities. Embeddings are generated by an async background job triggered on create/update (see `00_overview.md` search convention).

```sql
-- Topics
ALTER TABLE topics
  ADD COLUMN search_vector  tsvector,
  ADD COLUMN embedding      vector(768);  -- dimensions match SEARCH_EMBEDDING_MODEL
...
-- course_path_nodes (course names)
ALTER TABLE course_path_nodes
  ADD COLUMN search_vector  tsvector,
  ADD COLUMN embedding      vector(768);
CREATE INDEX idx_cpn_search_vector ON course_path_nodes USING GIN(search_vector);
CREATE INDEX idx_cpn_embedding     ON course_path_nodes USING hnsw(embedding vector_cosine_ops);
...
CREATE INDEX idx_org_embedding     ON organizations USING hnsw(embedding vector_cosine_ops);

-- teacher_profiles (bio, subjects)
ALTER TABLE teacher_profiles
  ADD COLUMN search_vector  tsvector,
  ADD COLUMN embedding      vector(768);
CREATE INDEX idx_teacher_search_vector ON teacher_profiles USING GIN(search_vector);
CREATE INDEX idx_teacher_embedding     ON teacher_profiles USING hnsw(embedding vector_cosine_ops);
...
> **Note on dimensions:** `vector(768)` is a placeholder. The actual dimension must match the configured `SEARCH_EMBEDDING_MODEL`. Update this in the Alembic migration once the embedding model is selected.

---

## 6a. Content Extraction Cache

Async text extraction for hAITU topic-doubt context and pgvector indexing is tracked per `topic_contents` item.

```sql
ALTER TABLE topic_contents
  ADD COLUMN text_extracted    TEXT NULL,
...
**Lifecycle:**
- Row created → `extraction_status = 'pending'`.
- Async background job runs PDF text extraction / plain-text passthrough → sets `text_extracted`, `extraction_status = 'complete'`, `extracted_at = now()`.
- Any update to the `topic_contents` row resets `extraction_status = 'pending'` and re-triggers the job.
- If extraction fails (corrupt PDF, timeout) → `extraction_status = 'failed'`; hAITU uses topic title only as fallback context.
