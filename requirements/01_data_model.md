...
categories
  └── course_path_nodes   (self-referential tree, arbitrary depth)
        └── topics              ← leaf nodes only
              └── topic_contents   (PDF, video, text)
                    └── topic_content_chunks   ← RAG chunks (generated by ingestion pipeline)
```
...
class CoursePathNode(BaseModel):
    ...
    name: str
    node_type: str              # free string, max 50 chars (see note below)
...
> **`node_type` is a free string** — not a strict enum. Two tiers:
> - **Reserved** (system behaviour depends on these — cannot be repurposed): `grade`, `subject`
> - **Default** (shown as suggestions in the curriculum builder UI): `course`, `chapter`, `module`, `section`, `unit`, `week`
> - **Custom** — any non-empty string up to 50 chars (e.g. `sprint`, `strand`, `track`)
>
> Service layer constants: `RESERVED_NODE_TYPES = frozenset({'grade', 'subject'})` and `DEFAULT_NODE_TYPES = frozenset({'course', 'chapter', 'module', 'section', 'unit', 'week'})`.
> The `grade` type drives topic locking (a topic is locked if its nearest `grade` ancestor represents a grade above the student's). The `subject` type drives subject-level analytics grouping.
...
`topic_content_chunks` ❌ NEW — vector chunks for hAITU retrieval. **Generated and managed by the LlamaIndex ingestion pipeline** — application code does not write to this table directly.

```sql
CREATE TABLE topic_content_chunks (
    id                UUID         PRIMARY KEY,
...
**Why a separate table (not self-referential on `topic_contents`):**
One content item produces many chunks (1:N). Adding chunks as rows on `topic_contents` (e.g. with `parent_content_id`) would require `WHERE parent_content_id IS NULL` guards across all existing queries. Separate table = one migration, zero footgun risk.

**Only minimal linking metadata** (`topic_id`, `topic_content_id`, `chunk_index`) is stored here. All rich metadata (source filename, content type, uploader, timestamps) stays in `topic_contents` and is accessed via join at query time.

**Prerequisite:** `pgvector` extension must be enabled on the PostgreSQL instance before this migration.

**Video content:** Only `pdf` and `text` content types produce chunks. Video items have no extractable text — `extraction_status = 'skipped'` for video rows. Topics with only video content will have no chunks; hAITU falls back to `text_extracted` truncation (BR-AI-010).
