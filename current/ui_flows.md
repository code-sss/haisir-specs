...
> No UI-flow change this phase — Phase 7.5 was deploy/infra-focused; the frontend range carries no application code changes at all (nanoid version pin only).
>
...
- Screen: `/parent/curriculum/[node_id]/topics/[topic_id]` (P-topic) — Topic Content Manager. Header: inline-editable topic title (`RenameTopicInline`, shared with admin) + "Back to curriculum" link (Phase 5 G7-patch-7/8 — now carries `?nodeId=<id>` to restore tree selection on return; the query param is named `nodeId`, not `node`, since the WAF's Coraza RCE ruleset (932236) flags bare shell/interpreter binary names like `node` as a query **parameter name** and blocks the request). Body: `TopicContentSection` — the same content-management feature module used by the admin `TopicRow`, parameterized via a `parentContentAdapter` (owner-scoped API calls instead of platform-scoped). Supports the full instant (video/text) + extraction (PDF/image) content flows, extraction job strip, and toast notifications on job completion, identical UX to the admin content manager — including the Phase 6.5 View button / Draft-Published pill / `ContentPublishControl` / markdown live-preview editor described under Admin Board Content Manager above (same components, same code, adapter-swapped API calls), and the Phase 6 `IndexingStatusPill` + Retry button per content row (see "Also complete 2026-07-26" in `progress.md` — `useContentManagement` wires the retry mutation, polling adapts 2s/10s/off based on active-indexing state, pulse animation respects `prefers-reduced-motion`).
  - Key behaviour: `content-management` is a new shared feature module (`add-content-modal`, `content-item-row`, `delete-content-dialog`, `jobs-strip`, `topic-content-section`, `publish-control`, `indexing-status-pill`) extracted so admin and parent consume one implementation via an adapter interface rather than two drifting copies.
  - API: `GET/POST /topics/{topic_id}/content` (Phase 5 G7-patch-6 — `GET` was entirely missing before this fix, 405ing the content list and making saved content look lost), `PATCH/DELETE /topic-contents/{content_id}`, `PATCH /topic-contents/{content_id}/publish` (Phase 6.5), `POST /topic-contents/{content_id}/retry-indexing` (Phase 6), `POST/GET/DELETE /api/parent/curriculum/topics/{topic_id}/extraction-jobs` (+ retry).
...
- Screen: `/add-exam` — Multi-field exam authoring form with question builder for MCQ and paragraph questions. Create or edit mode based on `template_id` query param. Exam settings include an **"Essay grading mode"** dropdown (`auto_release` = grades published to students immediately after AI grades; `manual_release` = instructor approves before publishing); defaults to `auto_release`. Essay questions show: **"Auto-grade with AI" checkbox** (defaults to `true`; when unchecked, AI grading pipeline skips the question on submit); **"Model answer" textarea** (prose answer shown to students after grade release, stored as `model_answer`); **"Mark scheme / Rubric" textarea** (grading criteria for reference, stored as `explanation`). Both model answer and mark scheme are optional. JSON import/export also serialises `model_answer` per essay question. **"Apply topic to all questions"** control (Pre-Phase-5 G2, 2026-07-05) — a topic `<select>` + "Apply to all questions" button above the questions section; stamps the chosen `topic_id` (or clears it) onto every standalone and paragraph-embedded question in one click, so exam authors don't have to tag each question individually for mastery tracking to fire.
  - **Question and option images upload to the server (Phase 7 G3.5, frontend `76f8bcc`).** `QuestionEditor` calls `useExamImageUpload` → `POST /api/exams/images` and stores the returned relative URL on `image_url`. The previous `FileReader.readAsDataURL` path — which embedded the whole image as a base64 `data:` URI in the exam-authoring payload — is deleted. On upload failure the existing `image_url` / `options_obj` is left **unchanged** and the error surfaces from hook state, so a failed upload never blanks a working image. The handler reads `questionRef.current` rather than the closure's `question`, so edits made during the upload window aren't clobbered by a stale closure on completion. Previews render as a plain `<img class="img-fill-contain">` instead of `next/image` with an inline `style` — inline styles are unavailable under the enforced CSP (BR-CSP-002).
...
- Row: **TopicRow** — individual topic card with: inline rename (click title → RenameTopicInline), draft/live status toggle ("Set live" / "Set draft" button calling `PATCH /api/topics/{id}`), and a delete (×) button opening DeleteTopicDialog. Renders a **content management section** (ContentItemRows with inline rename + provenance badges, "Add Content" button) and an **extraction jobs strip** below the content section. The jobs strip shows the last 3 extraction jobs (sorted newest-first) via `useExtractionJobs`; each job row shows filename, status label, pages_completed/pages_total progress, and Cancel/Retry actions. When a job transitions to `done`, the strip fires `onJobDone` which invalidates the topic contents query and shows a toast. When all jobs are terminal and 60 s have elapsed with no active jobs, polling stops. **Inline content rename**: `handleRenameContent` uses `useUpdateTopicContent` mutation; `renamingContentId` state tracks the in-flight item; `isTitleSaving` prop passed to `ContentItemRow` for disabled state.
  - API: `GET /api/topics-contents/{topicId}`, `POST /api/topics-contents/`, `PATCH /api/topics-contents/{id}`, `PATCH /api/topics-contents/{id}/publish` (Phase 6.5), `GET /api/topic-contents/{id}/file` (Phase 6.5, per-content file serve), `DELETE /api/topics-contents/{id}`, `GET /api/admin/topics/{topicId}/extraction-jobs`, `DELETE /api/admin/extraction-jobs/{jobId}`, `POST /api/admin/extraction-jobs/{jobId}/retry`

- Row: **ContentItemRow** — one row per content item: type icon (🎬 video / 📄 pdf / 🖼️ image / 📝 text / ❓ question / 💬 question_answer — `image` added Phase 6.5), **inline title rename** (title rendered as a `<button>`; click → switches to `<input>` with `autoFocus`, max 200 chars; Enter saves, Escape/blur cancels; disabled while save is in-flight), **provenance badge** (if `item.provenance != null`, renders `✨ from {source_filename} · p.{page_no}` pill below title with tooltip “Edits don’t affect the audit record”), optional description (truncated, full text in `title` tooltip), order badge, a **Draft/Published pill** (Phase 6.5 — reads `item.visibility_status`), **View** button (`pdf`/`image` rows — opens the shared `ContentViewer` in a modal; replaces the Edit button, since there's no text body to edit on a raw file), **Edit** button (`video`/`text` rows only), × delete button. Rename callback `onRename(item, title)` and `isTitleSaving` prop passed from `TopicRow`.

- Control: **ContentPublishControl** (Phase 6.5, `content-management/components/publish-control.tsx`, shared by admin `TopicRow` and parent `TopicContentSection`) — sits above the content list for a topic. For an extraction group (a raw `pdf`/`image` row plus its sibling extracted `text` row(s)): renders two buttons, **"Publish as Document"** and **"Publish as Text"**, each firing one `PATCH .../publish` call against the respective row's id — the server enforces mutual exclusivity (BR-EXT-037), so clicking one flips the other side to Draft in the same request. For a standalone row (video/text created via the instant path, or an unrelated group): a single **"Publish"** button. There is no unpublish action — drafting one side always happens as a side effect of publishing the other.

- Modal: **View dialog** (Phase 6.5) — a `<dialog>` opened by the ContentItemRow View button, rendering the shared `ContentViewer` for the single selected item. Header (title + × close) is `position: sticky` inside a `max-height`/`overflow-y: auto` box so long content scrolls internally rather than the page behind it.

- Modal: **AddContentModal** — native `<dialog>`; `mode: 'create' | 'edit'`. **4-chip type selector**: PDF, Image(s), Video URL, Text (chips disabled in edit mode). For PDF/Image chips: drag-and-drop file zone (or click to browse, accepts `application/pdf`/`image/*`); shows file list with remove buttons; cost estimate preview (pages × per-page rate) with a confirmation checkbox before upload is enabled; up to 5 files per submission; validates size ≤ 50 MB and MIME type. **Upload-closes-immediately**: on submit with files, modal closes at once and background upload fires via `extractionHook.uploadFiles()`; pseudo-jobs appear in the topic strip immediately. For Video URL chip: URL field + title/description/order fields, same Zod validation as before — video playback uses the official YouTube IFrame Player API / Vimeo Player SDK (Phase 6.5), not a raw `<iframe src>`; an embed error falls back to a "Watch on YouTube/Vimeo" external link. For Text chip: title/description/order + a **markdown editor with live preview** (Phase 6.5 — textarea + rendered pane via the shared `MarkdownText` component, replacing a plain textarea; same rendering the student viewer uses, so the uploader's preview matches exactly what gets published). In edit mode, only Video/Text are editable; content_type is immutable. `useFocusTrap` traps keyboard focus.

- Component: **ContentViewer** (Phase 6.5, promoted to `src/shared/components/content-viewer/` — pure move, no behaviour change from its prior student-only location) — dispatches per `content_type`: `pdf` → `SecurePdfViewer` (lazy-loaded, fetches via the per-content file endpoint); `image` → a new `ImageViewer` (same auth-blob-fetch pattern as the PDF viewer, since a raw `<img src>` can't carry the `X-Current-Role` header the file endpoint requires); `video` → the SDK player described above; `text`/`question`/`question_answer` → `MarkdownText`. Used by the admin/parent View dialog above and by the student topic content page.
...
- Screen: `/csp-report` — `POST` accepts CSP violation reports; returns 204. No UI. **Now actually persists reports** (Phase 7 G5) — it previously discarded them, which made a Report-Only soak structurally incapable of reporting anything. Report body size is bounded (BR-CSP-008). APISIX route `25-csp-report.json`.
- Screen: `/*` (not-found) — 404 fallback page.

### Cross-cutting: CSP (Phase 7 G5 — enforced in production)
Not a screen, but it constrains every screen above.

- A **per-request nonce** is minted in `haisir-frontend/src/proxy.ts` and paired with `'strict-dynamic'`. `Content-Security-Policy` (enforcing) in production; `Content-Security-Policy-Report-Only` retained in development as the CI regression surface. `report-uri` is live in both modes.
- **Every route is dynamically rendered** (BR-CSP-010), enforced by a CI assertion — a build-time-prerendered page cannot receive a per-request nonce. Previously only 15 of 27 pages carried `force-dynamic`; the opt-in now happens once in the root layout (`c66942a`).
- Two documented relaxations: `style-src-attr 'unsafe-inline'` (runtime-computed layout values no nonce can cover — `style-src` itself stays nonce-gated, locked by a unit test) and `'wasm-unsafe-eval'` (required by `pdfjs-dist`, which the PDF content viewer uses).
- Practical consequence for anyone adding UI: **no inline `<style>`/`style=` attributes and no inline scripts.** Existing inline styles were moved to CSS Modules / `globals.css` (`adcd9d3`, `50aa168`), and `zod` is configured jitless in `instrumentation-client.ts` because its JIT path needs `eval`.
- `X-XSS-Protection` is `0` across all four APISIX plugin configs; the gateway carries a backstop CSP scoped to **non-HTML routes only**, so it never collides with the render-side policy.
...
- `useExamReviewChat(attemptId)`: loads pattern analysis **and the persisted thread** on mount, both via the same abort controller; resets all state (including `sentRef` and the abort controller) on `attemptId` change; manages `messages: ExamReviewChatMessage[]`, `isLoading`, `error`, `patternAnalysisLoaded`, `failedMessageId`; exposes `send(message, questionId?)`, `retry()`, `explainQuestion(questionId, questionNumber)`, `clearError()`.
